-- drop procedure dbo.SaveNakl

CREATE procedure dbo.SaveNakl
...
as 
declare 
  @ids varchar(max), @inMarsh99 bit, @DelivGroup INT,  @Box int, @TekId int, 
  @RefTekId int, @NewTekId int, @Nds INT, @Extra decimal(7,2), @RetExtra decimal(7,2),
  @ND datetime, @SourDate datetime, @TM char(8), @Stfdate datetime, @DocDate DATETIME,
  @StfNom varchar(17), @DocNom varchar(20), @SourNnak INT, @Qty decimal(10,3), 
  @BoxQty decimal(10,2), @Weight decimal(10,2), @SP decimal(10,2), @SC decimal(10,2), 
  @price decimal(14,4), @TaraAct char(2), @SerialNom INT, @Nnak int, @ContrTip SMALLINT,
  @KolErrorSklad INT, @Marsh int, @Sklad INT, @Sk50present bit, @EffWeight float, @Cost MONEY,
  @RemarkKassa varchar(50), @RemarkKassa2 varchar(20), @gpOur_ID int, @Ncod int, @mhid INT,
  @TekDay datetime, @DatnomOffset bigint, @NaklCount int, @NewNcId INT,
  @Done bit, @SPsm decimal(10,2), @worker bit, @gpName varchar(255), @DepID int ,
  @dn0 bigint, @dn1 bigint, @master int, @flgExclude bit, @flgExclSausage bit, @DocType SMALLINT,
  @stfnom_upd varchar(17), @num_except bit, @PricePrecision smallint, @CountZK int
begin
--  set transaction isolation level snapshot
  ...
  create table #F(tekid int, Kol decimal(10,3), Cost decimal(13,5), Price decimal(13,5), datnom bigint)
  ...
  set @num_except=0
  --  if @master in (37008,41782,42547,43363) and @stip<>4 set @num_except=1 
  ...
  BEGIN
    ...
    BEGIN TRANSACTION SaveNewNakls 
      ...
      declare CurBack cursor local fast_forward for 
      select reftekid,qty,price,Sklad,EffWeight,cost,RefDatnom
      ...
      SET @OldCount=(SELECT COUNT(*) FROM tdvi)
      ...
      SET @NewCount=(SELECT COUNT(*) FROM tdvi)
      PRINT('[SaveNakl]: до операции вставки в TDVI '+CAST(@OldCount AS VARCHAR)+' строк, после вставки '+CAST(@NewCount AS VARCHAR))
      ...
      WHILE (@@FETCH_STATUS=0)
      BEGIN
        PRINT('[SaveNakl]: Cursor CURDELIV2, @DelivGroup='+cast(@DelivGroup as varchar))
        -- insert into Log_temp(mess,DatNom) VALUES('DelivGroup=',@DelivGroup)	
        -- Проход по списку накладных:   
        
        select @Our_ID=dc.Our_id, @Srok=dc.srok, 
          @ContrTip=dc.ContrTip,  @gpOur_id=ISNULL(dc.gpOur_ID,0), @ag_id=dc.ag_id 
        ...
        if @stip=4 begin
          ...
          if @gpOur_ID=0 set @gpOur_ID = (select max(tdvi.DCK) from tdvi inner join Zakaz Z on tdVi.id=z.tekid where z.CompName=@CompName)
          if not exists (select 1 from defcontract where contrtip=5 and dck=@gpOur_ID) set @KolError=@KolError | 1024
        end
        ...
        if @RefDatNom>0 
          select @stip=c.stip, @dck=c.dck, @RetExtra=c.extra, @Actn=c.Actn,
            @Frizer=c.Frizer, @Tara=c.Tara 
            from nc c where c.datnom=@RefDatNom
        else
          set @SPsm=isnull((select sum(sp) from nc where datnom >= @dn0 and datnom <= @dn1 and sp>0 and b_id=@b_id),0) 
                   
        select  
          @BoxQty=ISNULL(sum(z.Qty/VI.minp/vi.mpu),0),
          @Weight=ISNULL(sum(z.Qty*z.effWeight),0),
          @SP=ISNULL((1.0+isnull(@Extra,0)/100.0)*sum(z.Price*z.Qty),0),
          @SC=ISNULL(sum(z.Cost*z.Qty),0)
        ...
        print('[SaveNakl]: Временный фиктивный @datnom = '+cast(@datnom as varchar))
        ...
        select @TekDay=cast(val as datetime), 
          @DatnomOffset=cast(Comment as bigint) from Config where Param='DatnomOffset'
          IF @Datnomoffset IS NULL SET @Datnomoffset=0

        if cast(@TekDay as datetime)<>dbo.today()  or (@Datnomoffset=0) begin
          PRINT('SaveNakl]: @TekDay='+CAST(@TekDay AS VARCHAR)
            +', @Datnomoffset='+CAST(@Datnomoffset AS varchar)+' судя по данным из Config. Подозрительно! Будем пересчитывать DatnomOffset')
          set @NaklCount=ISNULL((select count(*) from nc where ND=dbo.today()),0)
          PRINT('SaveNakl]: @NaklCount='+CAST(@NaklCount AS VARCHAR))
          if @Naklcount=0
            set @DatnomOffset=dbo.fnDatNom(dbo.today(),0) - ISNULL((select max(ncid) from nc),0) 
          else set @DatnomOffset = (select top 1 datnom-ncid from nc order by datnom desc)
          update Config set val=convert(varchar, dbo.today(), 104), Comment=@DatnomOffset where Param='DatnomOffset'
        END

        PRINT('[SaveNakl]: DatnomOffset='+CAST(@Datnomoffset AS varchar))


        --Изменение срока консигнации. Надо ли это?
        ...
        if @inMarsh99=1 or @worker=1 or @stip=5 begin
          ...
          select @mhid=isnull(mhid,0) from nc where datnom=@RefDatnom
          set @ids=cast(@datnom as varchar)+'10#'
          if @mhid<>0 --нужно ли вставлять в маршрут
             PRINT('[SaveNakl]: вроде надо вставлятьв  маршрут, вызывать NearLogistic.MarshRequetOperations')
             -- exec NearLogistic.MarshRequetOperations @ids,@mhid,@op,0
        end
        
        

	print('[SaveNakl]: Вставка в NC @DCK='
	+cast(@DCK as varchar)+', временный @Datnom='
    +cast(@Datnom as varchar)+', @DatnomOffset='
    +cast(@DatnomOffset as varchar))

       
        insert into NC(ND, datnom,StartDatnom,B_ID,Fam,Tm,OP,SP,SC,Extra,
          Srok,OurID, Frizer,ag_id,stfnom,stfdate, Remark,Printed,BoxQty,
          Actn,CK,Tara, RefDatnom, Done, Izmen, RemarkOp, DayShift, Comp, 
          DCK, B_ID2, NeedDover, DoverM2, DocNom, DocDate, STip, gpOur_ID, mhid)
        values (@ND,@datnom,@Datnom,@B_ID, @Fam, @TM, @OP, isnull(@SP,0), isnull(@SC,0),@Extra,
          @Srok,@Our_ID, 0, @ag_id,@stfnom,@stfdate, @Remark,0,@BoxQty,
          @Actn,@CK,0, @RefDatNom, 0, 0, @remarkop, @DayShift, @CompName, 
          @DCK, @B_ID2, @NeedDover,@NeedDover2, @DocNom, @DocDate, @Stip, @gpOur_ID, @mhid)
        if @@Error<>0 set @KolError=@KolError | 1     
        set @NewNCID = scope_identity()

  PRINT('Что-то вставлено в NC, временный @datnom='+cast(@datnom as varchar)
  +', @NewNCID='+cast(@NewNCID as varchar)+', @Num_Except='+cast(@Num_Except as varchar)
  +', @@Error='+cast(@@Error as varchar)
  +', @DatnomOffset='+cast(@DatnomOffset as varchar))
 ...
 print('Подготовка к update NC, @@Error='+cast(@@Error as varchar))
 PRINT('@NewNCID='+cast(@NewNCID as varchar))
 print('@Datnomoffset='+cast(@Datnomoffset as varchar))
 SET @Datnom=@NewNCID+@DatnomOffset
       ...
       print('Update NC выполнен так или иначе, @@Error='+cast(@@Error as varchar)
          +', NC.Datnom='+CAST(@Datnom AS VARCHAR))
       ...
       if @@Error<>0 set @KolError=@KolError | 2
        ...
        insert into NV(hitag,datnom,tekid,price,
           OrigPrice,cost,Kol,sklad,kol_b,ag_id, Unid, OrigUnid, K)
        select z.hitag, @datnom, z.tekid,round(z.price, @PricePrecision), 
          vi.price as OrigPrice, z.cost, 
          z.qty, vi.sklad, 0, @Ag_ID, Z.unid, vi.unid AS OrigUnid, z.K
        from  
          Zakaz z 
          inner join tdVI vi on vi.id=z.tekid
          inner join SkladList SL on SL.SkladNo=vi.Sklad
        ...
        truncate table #F
        insert into #F(tekid, Kol,Cost,Price, datnom) 
          select tekid, kol,Cost,Price, @datnom from nv where datnom=@datnom

          -- И на всякий случай пересчитаем суммы по накладной:
        select 
          @SP=ISNULL(round(sum(kol*price*(1.0+0.01*@Extra)),2), 0),
          @SC=ISNULL(ROUND(sum(kol*cost),2),0)
          from #F where datnom=@datnom
          
        -- set @SPsm=isnull(@spsm,0)
          
        -- set @Done=iif(((@SPsm+@SP>0) or @SP<0 or @DepID=3 or @DepID=43 or @DepID=26 or @worker=1 or @flgExclude=1 or @RefDatNom>0),1,0)
        ...
        if (@RefDatNom>0) and (@Actn=0) and (@DocType=2) BEGIN
        ...
        BEGIN
          ...
          declare CurSell cursor fast_forward 
            ...
            from Zakaz join #f on Zakaz.tekid=#f.tekid
...
--              exec dbo.SendNotifyMail 'it@tdmorozko.rusargon5000@yandex.ru','Error on SaveNakl procedure',@ss, 0, ''
--              exec dbo.SendNotifyMail 'it_info@tdmorozko.ru','Error on SaveNakl procedure',@ss, 0, ''
        ...
        BEGIN
          ...
          WHILE (@@FETCH_STATUS=0) BEGIN
            exec ProcessSklad 'Скла', @TekId, null, @Sklad,null, null, @Qty, @OP, @CompName,
                 0, 0, 0, 0, 0, null, 
                 'Перемещение по возврату ()', 
                 @NewTekid, @SerialNom, @KolErrorSklad, null, null, null
            fetch next from CurDiff into @tekid, @Sklad, @Qty
   ...
   if @KolError  = 0 begin
     ...
     --select @Datnom
     print '[SaveNakl]: Завершение. Ошибки нет. Новый @Datnom = ' + Cast(@datnom as varchar)
   end
   else begin
     print '[SaveNakl]: Завершение. Ошибка процедуры. Datnom = ' + Cast(@datnom as varchar)
     set @QtyNakl=0  
     ROLLBACK 
   ...
   IF OBJECT_ID('tempdb.#f') IS NOT null  drop table #f
 end -- if EXISTS(select * from Zakaz where CompName=@CompName) 

...
CREATE procedure dbo.ProcessSklad_OLD
  ...
  ********************************************************************/  
   ...
   *****************************************************************/  
    ...
    if @Kol<=0 set @kolError=64;
      ...
      set @LastID=-round(RAND()*100000000,0)-1
      ...
      select
        ...
        p.hitag, @sklad/*p.Sklad sklad*/, p.Cost cost, 0 as Nalog5, nm.MinP, nm.Mpu, 
      ...
      set @NewID=(select @LastID-max(p.nomer) from ParamSklad p where p.Comp=@comp and p.Nomer>0);
   ...
   *****************************************************************/  
    ...
    print('ProcessSklad: Вход');

    set @Locked=0;
    
    if EXISTS(select p.* from ParamSklad p inner join TDVI V on V.id=p.Id 
    where p.Comp=@Comp +'_1')--and p.Nomer>0 and v.morn-v.sell+v.isprav-v.remov<=0)
    set @KolError=64;
    else begin    
      ...
      if @MaxId is null begin
       ...
       select
         ...
         @Hitag, @sklad/*p.Sklad*/, p.Cost, 0, @MinP, @Mpu, @Sert_Id, '5', 0,0,p.qty,
         0,0,@dater, @srokh, @country, 0, @units, @locked, @ncountry,
       ...
       if @@Error<>0 set @KolError=@KolError + 32;
      print('ProcessSklad: @KolError='+cast(@kolError as varchar));
     end;     
  end;

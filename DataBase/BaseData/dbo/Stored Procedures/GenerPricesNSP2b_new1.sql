-- Команда от главбуха: 7-й фирме продавать ТОЛЬКО товары 7-й фирмы. 17.06.2016
CREATE procedure dbo.[GenerPricesNSP2b_new1] @DCK INT, @SkladList varchar(3000)=NULL,
  @Datnom int=0, 
  @Action tinyint=null,
...
-- Исполнение команды главбуха, 17.06.2016
-- Нет, отменено по команде главбуха 22.06.2016!
set @OnlyMorozko=0;
--if @ContrTip<>7 begin
--  if @Action=5 begin
--    SET @OurID=(SELECT P.Our_ID FROM Person P INNER JOIN AgentList A ON A.P_ID=P.P_ID WHERE a.ag_id=@Ag_ID);
--    if @OurID=7 set @OnlyMorozko=1;
--  end;
--  else if @Action=0 and @DCK=7 set @OnlyMorozko=1;
--  else if @Action=0 and @dck is not null and @DCK<>7 and exists(select * from defcontract where dck=@dck and Our_id=7)
--    set @OnlyMorozko=1;
--end;
--print('@OnlyMorozko='+cast(@OnlyMorozko as varchar))

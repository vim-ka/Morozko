CREATE PROCEDURE dbo.GetCurrentNCDet_copy_copy
@datnom INT,
@isGroup bit=0,
@b_id int=0,
...
begin
	...
	begin
		...
		select 	nv.datnom, 
            ...
            isnull(inpdet.Id_vet_svid,0) Id_vet_svid,
/*
            STUFF((select '; ' + CAST(s.VetId AS VARCHAR) --коды
                FROM InpdetVetSvid s where s.id in (SELECT InpdetVetSvid.ID
                                      FROM InpdetVetSvid 
                                     WHERE InpdetVetSvid.inid = Inpdet.inId)
            for xml path(''))
            ,1,2,'') AS Id_vet_svid,*/
    ...
    LEFT JOIN Inpdet ON t.startid = Inpdet.id 
       ...
       --JOIN InpdetVetSvid ON Inpdet.inId = InpdetVetSvid.inId
...
/*
    ...
    JOIN Inpdet ON Inpdet.hitag = n.hitag 
       --JOIN InpdetVetSvid ON Inpdet.inId = InpdetVetSvid.inId

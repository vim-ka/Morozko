CREATE procedure dbo.AutoDivPlus_copy
@Comp varchar(30), 
@SkList varchar(100), 
@Hitag int, 
@OP int, -- оператор
...
PRINT('КОНТРОЛЬНАЯ ТОЧКА 5 ПРОЙДЕНА');  
    -- Какая была последняя поставка товара? 
    set @LastNcom=(select max(Ncom) from ParamSklad where Comp=@Comp and Hitag=@Hitag and Act='div-' and Ncom>0 and Ncod=@Ncod);
    if @LastNcom is null -- если номер поставки определить не удалось, ищем данные по поставщику в целом:
      set @LastID = (select max(i.ID) from inpdet i inner join Comman C on C.Ncom=i.Ncom where i.Hitag=@Hitag and c.Ncod=@Ncod);
    else -- а если удалось, то именно по этой поставке:
      set @LastID = (select max(ID) from inpdet where Hitag=@Hitag and Ncom=@LastNcom);

    -- Цена 1 кг в момент прихода, возможно, null, если вес не был указан:
    set @LCost1kg=NULL; -- вот эта инициализация нужна, ибо 
    set @LPrice1kg=NULL; -- SELECT ведь может и не сработать.

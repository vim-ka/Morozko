CREATE PROCEDURE dbo.[CalcNdsSumDatnomNewVer_new] @kassid int
AS
BEGIN
  ...
  select kassid, plata from kassa1 where sourdatnom in (select datnom from #TempNV) and oper=-2 and kassid<=@kassid and plata<>0
  order by kassid

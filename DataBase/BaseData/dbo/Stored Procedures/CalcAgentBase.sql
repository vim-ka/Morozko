CREATE procedure CalcAgentBase @day0 datetime, @day1 datetime
as
begin
  ...
  select sv.depId, d.DName, sv.ag_id as sv_id, e.AG_ID,  e.Fio, e.AgentPart, e.MainParent,e.GrpName,
    sum(e.GroupSP) as GroupSP,
    sum(e.GroupPlata) as GroupPlata,
    sum(e.GroupDohod) as GroupDohod
  from (
    select 
      al.Agentpart, #t.sp, #t.PayKoeff, al.AG_ID, P.Fio, gr.MainParent, G2.GrpName,
    ...
    group by
      al.Agentpart,#t.sp, #t.PayKoeff, al.AG_ID, P.Fio, gr.MainParent, G2.GrpName) E
  inner join AgentList SV on SV.ag_id=E.ag_id
  inner join Deps D on D.DepID=sv.depID
    group by sv.depId, d.DName, sv.ag_id, e.AG_ID, e.Fio, e.Agentpart, e.MainParent,e.GrpName
  order by sv.depId, e.AG_ID, e.Fio, e.Agentpart, e.MainParent,e.GrpName

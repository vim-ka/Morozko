CREATE PROCEDURE dbo.RollBackComman
@ncom int 
AS
declare @ErrReg int
...
set @tranname='RollBackComman'
begin tran @tranname
	set @ErrReg=0

	if exists(select * from nv where tekid in (select id from tdvi where ncom=@ncom)) 
  	set @ErrReg=@ErrReg+1
  else begin
    -- Новый код от 13.04.2018, очистка следов от Reassesment в Izmen и Kassa1:
    ...
    while @@FETCH_STATUS=0 BEGIN
      ...
      BEGIN
        ...
        fetch next from c1 into @Hrono, @remark, @comp, @DeltaCost, @IzmID
        -- Не забыть разблокировать в финальной версии: (хотя не надо, все равно эти строки будут стерты)
        -- update izmen set newcost=cost where izmid=@izmid
      end
    ...
    set @ErrReg=@ErrReg+@@error
  	...
  	--delete from [Log] where patindex('%Блок%',[tip])<>0 and cast(Param2 as int) in (select id from tdvi where ncom=@ncom)
    --set @ErrReg=@ErrReg+@@error
    
  	delete from tdvi where ncom=@ncom
    set @ErrReg=@ErrReg+@@error
  	
  	delete from izmen where ncom=@ncom
  	set @ErrReg=@ErrReg+@@error

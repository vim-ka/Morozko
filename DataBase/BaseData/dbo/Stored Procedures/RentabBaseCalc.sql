CREATE PROCEDURE dbo.RentabBaseCalc @day0 datetime, @day1 datetime, @ncod int, @ngrp int, @with_actn bit, 
...
--  select @cnt = count(*) from dbo.rentabcalc where ym_from = @ym_from and ym_to = @ym_to and ncod = @ncod and (ngrp = @ngrp or @ngrp = -1)  
  ...
  begin
    ...
    create table #rc(ym_from int, ym_to int, calctip int, ncod int, ngrp int, postvol int, obl_id int, 
    	cost numeric(10, 2), price numeric(10, 2), calcvid int, nds numeric(10, 2), postvol2 int, hitag int, ncod_p int)
    
    if @calctip = 1
    begin
      print convert(varchar, getdate())
      insert into #rc(ym_from, ym_to, calctip, ncod, ngrp, postvol, obl_id, cost, price, calcvid, nds, postvol2, hitag, ncod_p)
      ...
      1, avg(n.nds),
      sum(IIF(v.weight = 0, n.Netto, v.weight) * isnull(nv.kol, 0)) postvol2, nv.hitag, v.ncod 
    from
      dbo.nv nv
...
--      and o.obl_id in (1, 5, 7, 8, 14)
      ...
      group by g.mainparent, d.obl_id, v.ncod, nv.hitag, v.ncod 
    ...
    begin
      ...
      round(sum(nv.price * nv.kol * (1 + nc.Extra / 100)) / sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol), 2) price,
      1, avg(n.nds),
      sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol) postvol2, nv.hitag, v.ncod      
    from
      dbo.nv nv
      inner join dbo.nomen n on n.hitag = nv.hitag
      inner join dbo.gr g on g.Ngrp = n.ngrp
...
--      and o.obl_id in (1, 5, 7, 8, 14)
      ...
      and nc.Tara = 0 and nc.Frizer = 0      
      and g.Ngrp not in (0, 84, 86, 90)      
      group by g.mainparent, d.obl_id, d.pin, nv.hitag, v.ncod 
      having sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol) <> 0    
    end
...
--      and o.obl_id in (1, 5, 7, 8, 14)
      ...
      and nc.Tara = 0 and nc.Frizer = 0      
      and g.Ngrp not in (0, 84, 86, 90)      
      group by g.mainparent, d.obl_id, nv.hitag, v.ncod
      having sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol) <> 0    
    end

    insert into dbo.rentabcalc select * from #rc where (#rc.postvol <> 0 AND #rc.postvol2 <> 0)
  end

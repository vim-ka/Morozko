CREATE PROCEDURE dbo.MainVendorsList @Actual bit, @Bonus bit, @Holod bit, @Our_id varchar(50)='', @BnFlg smallint=2, @Contrtip varchar(50)='', @Type tinyint=0,
                                     @NDay datetime=null 
AS
BEGIN
  ...
  begin   
    ...
    select v.pin as Ncod, 
           ...
           v.bnflg as bnFlag,
           ISNULL(v.contact,'') as Contact,
           ISNULL(v.brPhone,'') as Phone,
    ...
    where (v.actual = @Actual or v.actual = 1)  
...
--          and ( or ct.dck=0)
    ...
    group by v.pin, v.brName, v.srok, v.bnflg,v.contact, v.brPhone,

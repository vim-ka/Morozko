CREATE PROCEDURE dbo.EloadSertifErrors
@nd1 datetime,
@nd2 datetime
AS
BEGIN
...
select 	n.Nd [Дата],
        ...
        cast(iif(exists(select 1 from defcontract dc inner join AgentList al on dc.ag_id=al.AG_ID where dc.pin=d.pin and al.DepID=3),1,0) as bit) [Сеть],
        ...
        --MobAgents.Mess      
from nc n
inner join Def d on n.b_id = d.pin
where d.tip = 1
      and n.datnom >= @dat1
      and n.datnom <= @dat2 
      and n.sp > 0 
      and (n.Remark like '%вет%' or
           ...
           exists(select 1 from defcontract dc inner join AgentList al on dc.ag_id=al.AG_ID where dc.pin=d.pin and al.DepID=3))
      and n.Marsh <> 99

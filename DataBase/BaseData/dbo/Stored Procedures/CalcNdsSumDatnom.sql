CREATE PROCEDURE dbo.[CalcNdsSumDatnom] @kassid int, @op int=0
AS
BEGIN
  ...
  where k.oper=-2 and (k.kassid=@kassid or (k.origrecn=@kassid and k.plata>0))
  ...
  union
  select kassid, plata from kassa1 where kassid=@kassid and plata<>0 and oper=-2
  union
  select kassid, plata from kassa1 where origrecn=@kassid and plata<>0 and oper=-2
  ...
  insert into  dbo.PrintBillLog (OP,  kassid, Comp) values (@OP, @kassid, host_name());
  
  select rank() over (order by name, nvid desc) as rank, nd, nvid, dbo.InNnak(datnom) as NNak, 
  dbo.DatNomInDate(datnom) as NNakDate, hitag, name, price, qty, ABS(sp) as sp, nds, round(pay,2) as Pay, ndssp, kassid, payall, tekid
  from #tempNV 
  where kassid=@kassid
  order by rank 

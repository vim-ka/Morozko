CREATE procedure dbo.GenerPricesNSP2b_NEW @DCK INT, @SkladList varchar(3000)=NULL,
  @Datnom int=0, 
  @Action tinyint=null,
...
declare @ctVendor tinyint, @ctGroup tinyint, @ctWares tinyint, @ctAll tinyint,
  ...
  @today datetime, @StartDay DATETIME


IF @Action=5 SET @OurID=(SELECT P.Our_ID FROM Person P INNER JOIN AgentList A ON A.P_ID=P.P_ID WHERE a.ag_id=@Ag_ID);
--else if @Action=0 and @DCK>0 set @OurID=(select Our_ID from Defcontract where DCK=@DCK);
...
begin
  ...
  create table #t (id int, sklad int, hitag int, Ncod int, ngrp int, [Parent] int, MainParent int, 
    ...
    Sert_ID int, Gtd varchar(100),NvId int); -- последние 8 полей, с Kol по NvID - для анализа накладных.
...
/****************************************************************/
  if @Action=6 and @VendDCK>0 -- задан список HITAG товаров:
    ...
    where 
      ...
      and S.Locked=0 and s.Discard=0 and s.safecust=1
      and v.Rest > 0
 else     
/****************************************************************/
  if @Action=6 and @VendDCK=0 -- задан список HITAG товаров:
    ...
    where 
     ...
     and s.Discard=0 and s.Equipment=0
     and v.Rest > 0 
     --and (@OurID=0 or V.Our_ID=@OurID)   
     and (s.skladno<100 or s.skladno>200)
     and (@FirmGroup=0 or fc.FirmGroup=@FirmGroup);
/****************************************************************/
  else if @Action=3
...
/****************************************************************/    
    ...
    insert into #t (id,sklad,hitag,ncod,ngrp, [Parent], MainParent, WEIGHT,Origprice, 
      flgWeight, Cost, Locked,Minp,Mpu, Rest,DCK,Kol,Kol_B,Country,DateR,SrokH,Sert_ID,Gtd,NvId,nmid)
    select 
      ...
      nv.price, nm.flgWeight,V.Cost, v.Locked, v.minp, v.mpu,
...
--  select #t.*, nm.[name], nm.FName, case when s.OnlyMinP = 1 then cast(1 as bit) else nm.OnlyMinP end OnlyMinP, nm.Nds,s.UpWeight,nm.netto,#bpl.LastPrice
 ...
 --исправление заказа
    ...
    select distinct @Ag_id, @B_ID, @DCK, cast(t.hitag as varchar(5)) +' '+ n.name,1
    from #LocalTDVI t join nomen n on t.hitag=n.hitag where t.id in (select id from #t where Disab=1);

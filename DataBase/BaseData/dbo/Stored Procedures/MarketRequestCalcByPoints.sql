--расчет статистики по акциям для точек
CREATE procedure dbo.MarketRequestCalcByPoints @MRID int, @nd0 datetime, @nd1 datetime, @flgDebug bit=0, @skipproclist bit=1 
WITH RECOMPILE
as
...
--print('КОНТРОЛЬНАЯ ТОЧКА 6 ПРОЙДЕНА')
  set @PlanVal=(select Planval from MarketRequestSlotMeh where mrid=@MRID and bonus = 0);
...
--select * from #nkk
  ...
  select 'результат' as Remark,
    ...
    sum(case 
      ...
      else #nkk.wgt end)/ @PlanVal as Rz, #nkk.hitag, sum(#nkk.kol) kol
  from 
    #nkk
  ...
  where 
    ...
    and d.mrid=@MRID
  group by     
    #nkk.depid, #nkk.ag_id, #nkk.b_id, #nkk.hitag
  having sum(case 
      when #nkk.tip=0 then #nkk.kol
      when #nkk.tip=1 then #nkk.kol/#nkk.Box
      when #nkk.tip=2 then #nkk.sp
...
--print('КОНТРОЛЬНАЯ ТОЧКА 7 ПРОЙДЕНА')
  ...
  insert into MarketRequestRes(mrid, ag_id, b_id, cnt, hitag, kol)
  select 
    ...
    cast(sum(case 
      ...
      when #nkk.tip=1 then #nkk.kol/#nkk.Box
      when #nkk.tip=2 then #nkk.sp
      else #nkk.wgt end)/ @PlanVal as int) as Cnt, #nkk.hitag, sum(#nkk.kol) -- Cnt округление вниз
  from 
    #nkk
    inner join MarketRequestDeps D on D.depid=#nkk.depid    
    inner join MarketRequestTovs T on T.hitag=#nkk.hitag

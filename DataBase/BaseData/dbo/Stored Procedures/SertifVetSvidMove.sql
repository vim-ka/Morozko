CREATE PROCEDURE dbo.SertifVetSvidMove @Our_ID_Owner INT, @Our_ID_Saler INT, @DateStart DATETIME, @DateEnd DATETIME, @HitagStr varchar(8000), @VetId INT
AS 
BEGIN
...
CREATE table #IDList (ID INT, VetId INT, IdIs INT, OurId INT, ProducerCodeID INT)
 insert into #IDList (ID, VetId, IdIs, OurId, ProducerCodeID) 
 select DISTINCT ISNULL(s.startid, 0) Id, InpdetVetSvid.VetId, InpdetVetSvid.IdIs, InpdetVetSvid.OurID, InpdetVetSvid.ProducerCodeID
...
select DISTINCT ISNULL(si.startid, 0) Id, InpdetVetSvid.VetId, InpdetVetSvid.IdIs, InpdetVetSvid.OurID, InpdetVetSvid.ProducerCodeID
  from  nv v 
...
--удаление
/*
IF EXISTS (SELECT 1 FROM InpdetVetSvid
             JOIN #IDList ON InpdetVetSvid.ID = #IDList.ID
            WHERE InpdetVetSvid.OurId = @Our_ID_Saler) 
DELETE InpdetVetSvid 
  FROM InpdetVetSvid
  JOIN #IDList ON InpdetVetSvid.ID = #IDList.ID
 WHERE InpdetVetSvid.OurId = @Our_ID_Saler
*/
...
--вставляем Id'ы, которым присвоены вет. св-ва, 
--с новым Id вет. св-ва, OurId и исследованиями
INSERT INTO InpdetVetSvid(Id, VetId, IdIs, OurID, ProducerCodeID)
(SELECT DISTINCT #IDList.ID, @VetId, #IDList.IdIs, @Our_ID_Saler, #IDList.ProducerCodeID  
  FROM #IDList
 WHERE #IDList.ID NOT IN
  (SELECT #IDList.id
     FROM #IDList
     JOIN InpdetVetSvid ON InpdetVetSvid.ID = #IDList.ID
     JOIN SertifVetSvid ON InpdetVetSvid.VetId = SertifVetSvid.Id_vet_svid 
    WHERE InpdetVetSvid.OurId = @Our_ID_Saler
      AND SertifVetSvid.Is_Del = 0
  )
)

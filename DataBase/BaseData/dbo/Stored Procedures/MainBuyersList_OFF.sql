...
CREATE PROCEDURE dbo.[MainBuyersList_OFF] @Actual bit
AS
BEGIN
...
begin 
    ...
    select distinct d1.pin, tip, gpName, gpIndex, gpAddr, gpRs, gpCs, gpBank, gpBik, gpInn,
           ...
           Contact,
           (select Sum(ISNULL(Oborot,0)) from Def where  MASTER=D1.pin  and Actual=1)+
           (select ISNULL(Sum(Sp),0)as Sm from NC where ND=@today
           and B_id in (select pin from Def where MASTER=D1.Master  and Actual=1)) as oborot,
           ...
           OKPO,OKPO2 ,NDSFlg,Ag_GRP, d1.Debit ,OGRN,tmDin,tmWork,OGRNDate ,SlAll,DisMinEXTRA,Tov,BNFlg ,
           Worker, TmPost,
           (select Sum(ISNULL(OborotIce,0)) from Def where  MASTER=D1.pin  and Actual=1)as OborotIce,
           SverTara,
           (select count(*) from Def D2 where D2.Master=D1.pin) X1,

           (select  IsNull(Sum(Sp+Izmen)-Sum(Fact),0)
                   from NC --nc
                   where Tara!=1 and Frizer!=1 and  Actn!=1 and 
                   B_id in (select pin from Def where MASTER=D1.pin and Actual=1)) as Duty,             
           (select IsNull(cast((GETDATE()- min(ND+Srok) )as int),0)
            from  NC --nc
            where B_id in (select pin from Def where MASTER=D1.pin  and Actual=1) and 
               Tara!=1 and Frizer!=1 and  Actn!=1 and
               (nc.SP+ISNULL(nc.izmen,0)-nc.Fact)>0.01 and ND+Srok+1<GETDATE() 
            )as NDDolg,
            ...
            where B_id in (select pin from Def where MASTER=D1.pin  and Actual=1)) as SPrice,
            /* (select Sum(Kol) from TaraDet td
             where B_id in (select pin from Def where MASTER=D1.pin  and Actual=1) and 
                selldate<=@today-@Cnt)*/ 0 as TaraKol
    from Def D1 join DefContract c on D1.pin=c.pin and (c.ContrTip=2 or c.ContrTip=7)
    where D1.Ncod is null and D1.Master=D1.pin and EXISTS(select pin from def where Actual=1 and MASTER=D1.pin)
    order by pin
     
    insert into  #TempTable
    select distinct D3.pin, tip, gpName, gpIndex, gpAddr, gpRs, gpCs, gpBank, gpBik, gpInn,
           gpKpp, brName, brIndex, brAddr, brRs,brCs, brBank, brBik, brInn, brKpp,
           ...
           OKPO,OKPO2 ,NDSFlg,Ag_GRP, d3.Debit ,OGRN,tmDin,tmWork,OGRNDate ,SlAll,DisMinEXTRA,Tov,BNFlg ,
    ...
    from Def D3 join DefContract c1 on D3.pin=c1.pin and (c1.ContrTip=2 or c1.ContrTip=7)
    LEFT JOIN
      (select  Sum(Sp+Izmen)-Sum(Fact)as Duty,B_id from  NC --nc
       where Tara!=1 and Frizer!=1 and Actn!=1 group by B_id)A on A.B_Id=D3.pin 
    LEFT JOIN
      (select cast((GETDATE()- min(ND+Srok) )as int) as NDDolg, B_Id, SUM(nc.SP+ISNULL(nc.izmen,0)-nc.Fact) as Overdue
       from  NC  where  ND+Srok+1<GETDATE() and
            (nc.SP+ISNULL(nc.izmen,0)-nc.Fact)>0.01  and Tara!=1 and Frizer!=1 and Actn!=1
       group by B_Id)B on B.B_id=D3.pin 
    LEFT JOIN
      (select Count(B_id) as CountFriz, sum(Price) as SPrice,B_id from Frizer where tip=0 
       group by B_id)C on C.B_id=D3.pin    
    LEFT JOIN
      (select Count(B_id) as CountOb, sum(Price) as SPrice,B_id from Frizer where tip!=0 
       group by B_id)E on E.B_id=D3.pin
    LEFT JOIN
      (select case when Sum(Kol)<0 then 0 else Sum(kol) end as TaraKol,B_id from TaraDet
       where selldate<=@today-@Cnt and datnom is not null   group by B_id)D on D.B_id=D3.pin
...
begin
   insert into  #TempTable 
    select distinct d1.pin, tip, gpName, gpIndex, gpAddr, gpRs, gpCs, gpBank, gpBik, gpInn,
           ...
           d1.Srok, d1.Actual, d1.Disab, d1.Extra ,LicNo ,LicWho ,LicSrok ,LicDate, Raz,BeginDate,
           Contact,
           (select Sum(ISNULL(Oborot,0)) from Def 
           where  MASTER=D1.pin  and tip=1) +
           (select ISNULL(Sum(Sp),0) as Sm from NC where ND=@today
           and B_id in (select pin from Def where MASTER=D1.Master and tip=1)) as oborot,
           Master,d1.Our_ID,Buh_ID, Reg_ID,Rn_ID,Obl_ID,Sver,NeedSver,
           Prior,d1.LastSver ,PeriodSver,ShortFam ,Torg12 ,TovChk, NetType,GrOt,Fmt,PrevAgID,
           OKPO,OKPO2 ,NDSFlg,Ag_GRP, d1.Debit ,OGRN,tmDin,tmWork,OGRNDate ,SlAll,DisMinEXTRA,Tov,BNFlg ,
           Worker, TmPost,
           (select Sum(ISNULL(OborotIce,0)) from Def 
           where  MASTER=D1.pin and tip=1)as OborotIce,SverTara, 
     (select count(*) from Def D2 where D2.Master=D1.pin) X1,
...
select distinct pin, brName, Oborot, Duty, Overdue, NDDolg,OborotIce, SPrice, CountOb,CountFriz,   

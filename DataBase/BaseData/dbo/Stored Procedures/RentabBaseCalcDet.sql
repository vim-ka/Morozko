CREATE PROCEDURE dbo.RentabBaseCalcDet @day0 datetime, @day1 datetime, @ncod int, @ngrp int, @with_actn bit, 
...
--  set transaction isolation level read uncommitted 
  ...
  set @dn2 = @dn1 + 9999 --dbo.InDatNom(9999, @day1)
...
/*    delete from dbo.rentabcalc where ym_from = @ym_from and ym_to = @ym_to 
    ...
    create table #rc(ym_from int, ym_to int, calctip int, ncod int, ngrp int, postvol int, obl_id int, 
    	cost numeric(10, 2), price numeric(10, 2), calcvid int, nds numeric(10, 2), postvol2 int, hitag int)
    ...
    begin
      ...
      select @ym_from, @ym_to, @calctip, @ncod, g.ngrp, 
...
--      and o.obl_id in (1, 5, 7, 8, 14)
      ...
      group by g.ngrp, d.obl_id, v.ncod
...
--      print convert(varchar, getdate())      
    ...
    begin
      insert into #rc(ym_from, ym_to, calctip, ncod, ngrp, postvol, obl_id, cost, price, calcvid, nds, postvol2, hitag)
      select @ym_from, @ym_to, @calctip, @ncod, g.ngrp, 
      ...
      round(sum(nv.price * nv.kol * (1 + nc.Extra / 100)) / sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol), 2) price,
      1, avg(n.nds),
      sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol) postvol2, nv.hitag      
    from
      dbo.nv nv
      inner join dbo.nomen n on n.hitag = nv.hitag
      inner join dbo.gr g on g.Ngrp = n.ngrp
...
--      and o.obl_id in (1, 5, 7, 8, 14)
      ...
      and nc.Tara = 0 and nc.Frizer = 0      
      and g.Ngrp not in (0, 84, 86, 90)      
      group by g.ngrp, d.obl_id, d.pin, nv.hitag
      having sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol) <> 0    
    end
...
--      and o.obl_id in (1, 5, 7, 8, 14)
      ...
      and nc.Tara = 0 and nc.Frizer = 0      
      and g.Ngrp not in (0, 84, 86, 90)      
      group by g.ngrp, d.obl_id
      having sum(IIF(v.weight = 0, n.Netto, v.weight) * nv.kol) <> 0    
    end

--    insert into dbo.rentabcalc select * from #rc
	select * from #rc
  end

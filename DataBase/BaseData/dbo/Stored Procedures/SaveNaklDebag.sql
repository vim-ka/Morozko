CREATE procedure dbo.SaveNaklDebag
...
as 
 ...
 declare @dn0 int, @dn1 int, @master int, @flgExclude bit, @flgExclSausage bit, @DocType smallint
begin
--  set transaction isolation level snapshot
  ...
  select @b_id=dc.pin, @ContrTip=dc.ContrTip from defcontract dc where dc.DCK=@dck;
...
-- create table nv_join (datnom int, refdatnom int, tekid int, reftekid int, weight decimal(10,3))
      ...
      for select distinct iif(@flgExclSausage=1,0,DelivGroup) as DelivGroup, MainExtra, StfDate, Stfnom, DocNom, DocDate, RefDatnom, DCK from Zakaz where Compname=@CompName order by DelivGroup, RefDatnom;
      open CurDeliv2; 
      ...
      if @RefDatNom=0 set @DocType=0
      else if exists(select * from zakaz where Compname=@CompName and qty>0) set @DocType=1
      else set @DocType=2

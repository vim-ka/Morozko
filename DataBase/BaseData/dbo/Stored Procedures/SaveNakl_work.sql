...
CREATE procedure dbo.SaveNakl_work
...
--  begin try
  ...
  select @b_id=dc.pin, @Our_ID=dc.Our_id, @Srok=dc.srok, @ContrTip=dc.ContrTip, @gpOur_id=dc.gpOur_ID, @ag_id=dc.ag_id 
...
--    SET TRANSACTION ISOLATION LEVEL READ COMMITTED
    begin transaction SaveNewNakls; 
      ...
      begin
        ...
        -- старый вариант:
        /*
        set @NewTekId=-round(RAND()*100000000,0)-1;
...
--		    set @NaklCount=(select count(*) from nc where datnom>=@dn0 and datnom <=@dn1)
        ...
        insert into NC (ND,datnom,B_ID,Fam,Tm,OP,SP,SC,
         ...
         from nv where datnom=@datnom;
          ...
          set @Done=iif(((@SPsm+@SP>1500) or @SP<0 or @DepID=3 or @DepID=43 or @DepID=26 or @worker=1 or @flgExclude=1),1,0);

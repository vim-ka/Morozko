CREATE procedure [dbo].[EloadTransDeliv] @day0 datetime, @day1 datetime
as
begin
  -- извлекаю важную информацию из накладных:
  create table #nc (nd datetime, datnom int, sp decimal(10,2), sc decimal(10,2), 
    VMaster int,  weight decimal(10,3),  Marsh int, NDM varchar(30), ag_id int, DepID int,mhid int );
...
*/
  ...
  select 
    ...
    isnull(case when M.Nd<='20120212' then M.OplataSum else M.OplataSum+M.PercWorkPay end,0) as Rashod,
    count(#nc.DatNom)
  from #nc left join NearLogistic.nlListPayDet M on M.mhid=#nc.mhid
  group by 
    #nc.nd, #nc.marsh, isnull(case when M.Nd<='20120212' then M.OplataSum else M.OplataSum+M.PercWorkPay end,0)

...
CREATE PROCEDURE dbo.OplataMarsh3 @ND1 dateTime,@ND2 datetime, @type varchar(3),
                                  @Op int,@Rem varchar(60)
AS
BEGIN
  ...
  begin
    insert into #Oplata (ND,Marsh,Napravl,plata,otherplata,
                     drId,DrName,Descript,crId,Phys,ScanND,VedNo,mhid,
    ...
    where /*ScanND>=@ND1*/VedNo=@VedOpl and ScanND<=@ND2 and mstate=2 and m.drid<>0 and ND>='04.04.11'
         and (A.crid=0 or A.PhysPerson>0 or A.CrId=7) and Dr.trId<>6 and DelivCancel=0 /*and km0<>0 and km1<>0*/
    order by m.drid                 
  end
  else
    if @type='CRR'
    begin
      ...
      from Marsh m
      left join
      (select V_id,v.crid,Cr.physperson,Cr.crName,Cr.crRs,Cr.CrInn from Vehicle v
       left JOIN
       (select physPerson,crid,crName,crRs,CrInn from Carriers)Cr on Cr.crid=v.crid
       )A on A.V_id=m.V_id
      left join
      (select Fio,drid
       from Drivers)Dr on Dr.drid=m.drid
      where /*ScanND>=@Nd1*/VedNo=@VedOpl and ScanND<=@ND2 and mstate=2 and m.V_id<>0 and ND>='04.04.11'
           and A.crid<>0 and A.PhysPerson=0 and A.CrId<>7 and DelivCancel=0 /*and km0<>0 and km1<>0*/
      order by A.crId
    end;
...
begin
    ...
    /* if @type='DRV' 
    begin    
          select distinct #Oplata.ND,Marsh,Napravl,A.OplataSm,A.otherOplSm,
                 plata,otherplata,
                 #Oplata.drId,DrName,Descript,crId,CrName,Phys,
                 case 
                   when ScanND is null then ''
                   else convert(Varchar(10),ScanND,4)
                 end as ScanND,#Oplata.VedNo,
                 case 
                   when #Oplata.VedNo=0 then v.Nom
                   else v.Nom
                 end newNom,mo.Nd as VedND
          from #Oplata
          join #Ved v on v.Nd=#Oplata.Nd and v.Msh=#Oplata.Marsh 
          left join MarshVedOpl mo on mo.VedNo=V.Nom
          left join
            (select Sum(Dots*DotsPay+dist*distPay+DrvPay-Bill) as OplataSm,
                    Sum(vetPay+waypay) as otherOplSm,marsh.drid
             from Marsh
             where mhid in (select mhid from #ved)
             group by DrID)A on A.drid=#Oplata.drid
          order by #Oplata.drId
    end
     else 
      if @type='CRR'
      begin  
          select distinct #Oplata.ND,Marsh,Napravl,A.OplataSm,A.otherOplSm,
                 plata,otherplata,
                 #Oplata.drId,DrName,Descript,#Oplata.crId,CrName,Phys,
                 case 
                   when ScanND is null then ''
                   else convert(Varchar(10),ScanND,4)
                 end as ScanND,#Oplata.VedNo,
          ...
          left join MarshVedOpl mo on mo.VedNo=V.Nom
          left join
          (select Sum(O.Oplata) as OplataSM,Sum(O.otherOpl) as otherOplSM,O.crid
           from (select ND,Marsh,(vetPay+waypay) as otherOpl,C.crid, 
                      case
                        when (Dots>25)and (CityFlg=3) then 
                                (IsNull(25*DotsPay+(Dots-25)*2*DotsPay+dist*distPay+DrvPay*Weight-Bill,0)) 
                        else (IsNull(Dots*DotsPay+dist*distPay+DrvPay*Weight-Bill,0))
                      end as Oplata  
                from Marsh
                left join
                (select V_id,v.crid,Cr.physperson from Vehicle v
                 left JOIN
                 (select physPerson,crid from Carriers)Cr on Cr.crid=v.crid
                )C on C.V_id=marsh.V_id
                where mhid in (select mhid from #ved))O
            group by CrId)A on A.crid=#Oplata.crID
            order by #Oplata.CrId
      end;*/
...
begin
  ...
  begin    
        select distinct o.ND,o.Marsh,Napravl,A.OplataSm,A.otherOplSm,
               ...
               o.drId,DrName,Descript,crId,CrName,Phys,o.Dist,o.weight,o.Dots,
               case
        ...
        (select Sum(d.OplataSum) as OplataSm,Sum(d.OplataOther) as otherOplSm,C.drid ,
         ...
         /* (select Sum(C.OplataSm) as OplataSm,Sum(C.otherOplSm) as otherOplSm,C.drid
           from (select case
                    when DotsPay<100 then
                       IsNull(Dist*DistPay+DrvPay*(weight+dopWeight)+Dots*DotsPay+
                                SpedPay-IsNull(Peni,0),0)
                    else
                      IsNull(Dist*DistPay+DrvPay*(weight+dopWeight)+DotsPay+
                           SpedPay-IsNull(Peni,0),0)
                  end as OplataSm,
                  IsNull(vetPay+waypay,0) as otherOplSm,marsh.drid
                from Marsh
                where mhid in (select mhid from Marsh where vedNo=@VedOpl) --(select mhid from #ved)
                )C
           group by C.DrID)A on A.drid=#Oplata.drid*/
        order by o.drId
  end
  else 
    if @type='CRR'
    begin  
        ...
        /*(select Sum(O.Oplata) as OplataSM,Sum(O.otherOpl) as otherOplSM,O.crid
         from (select ND,Marsh,IsNull(vetPay+waypay,0) as otherOpl,C.crid, 
                    case
                      when (Dots>25)and (CityFlg=3) then 
                            IsNull(25*DotsPay+(Dots-25)*2*DotsPay+dist*distPay+DrvPay*(Weight+dopWeight)
                                  +IsNull(PercWorkPay,0)-IsNull(Peni,0),0) 
                      else 
                            IsNull(Dots*DotsPay+dist*distPay+DrvPay*(Weight+dopWeight)+IsNull(PercWorkPay,0)
                                 -IsNull(Peni,0),0)
                    end as Oplata  
              from Marsh
              left join
              ...
              where mhid  in (select mhid from Marsh where vedNo=@VedOpl))O
          group by CrId)A on A.crid=#Oplata.crID*/
          order by o.CrId
    end;
end;

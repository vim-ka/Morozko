...
CREATE procedure dbo.SaveBackAndSale
  ...
  @OrigRemark varchar(255)='',
  @OrigRemarkOp varchar(255)='',
  @NcDCK int=0
as 
declare @StartDatnom int, @ND datetime, @Stip int, @TM char(8), @oldB_id2 INT,
...
begin
  ...
  Set @KolError=0;
  select @StartDatnom=StartDatnom, @Stip=stip, @oldB_id2=b_id2, @Actn=Actn, @Done=Done, @DocNom=DocNom, @gpOur_ID=gpOur_ID,
    @OrigRemark=Remark, @OrigRemarkOp=RemarkOp
  ...
  begin transaction
     ...
     values (@ND,@backdatnom, @StartDatnom, @OldB_ID, @OldB_ID2,  @OldFam,@TM,@OP,-@NewSP,-@NewSC,
     ...
     if (@KolError=0) begin
       ...
       values (@ND,@newdatnom, @StartDatnom, @NewB_ID, @oldB_ID2,@NewFam,@TM,@OP,@NewSP,@NewSC,
         @Extra,@Srok,@Our_ID, 0, 0, 
    ...
    exec dbo.RecalcNCSumm @NewDatNom;
    exec dbo.RecalcNCSumm @BackDatnom;

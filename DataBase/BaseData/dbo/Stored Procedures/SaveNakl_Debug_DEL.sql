...
CREATE procedure dbo.SaveNakl_Debug_DEL
...
as 
 ...
 declare @KolErrorSklad int, @TotalSP decimal(12,2)
 ...
 declare @dn0 int, @dn1 int, @master int, @flgExclude bit, @flgExclSausage bit
begin
  set @KolError = 0 
  ...
  begin try
  create table #summ(datnom int, sp decimal(12,2)); -- список накладных с их суммами

  create table #F(tekid int, Kol int, Cost decimal(13,5), Price decimal(12,2), datnom int);
...
--    SET TRANSACTION ISOLATION LEVEL SERIALIZABLE
    ...
    begin transaction SaveNewNakls; 
      ...
      for select distinct iif(@flgExclSausage=1,0,DelivGroup) as DelivGroup, MainExtra, StfDate, Stfnom, DocNom, DocDate from Zakaz where Compname=@CompName order by DelivGroup;
      open CurDeliv2; 
      fetch next from CurDeliv2 into @DelivGroup,@Extra,@StfDate,@StfNom, @DocNom, @DocDate;
      WHILE (@@FETCH_STATUS=0)  BEGIN
        print 'Cursor CURDELIV2, @DelivGroup='+cast(@DelivGroup as varchar(8))
...
--		    set @NaklCount=(select count(*) from nc where datnom>=@dn0 and datnom <=@dn1)
        ...
        insert into NC (ND,datnom,B_ID,Fam,Tm,OP,SP,SC,
                       ...
                       Comp, Sk50present, DCK, B_ID2, NeedDover, DoverM2, DocNom, DocDate, STip, gpOur_ID)
         values (@ND,@datnom,@B_ID,@Fam,convert(char(8), getdate(),108),@OP,@SP,@SC,
                 ...
                 @DayShift, @CompName, @Sk50present, @DCK, @B_ID2, @NeedDover,@NeedDover2, @DocNom, @DocDate, @Stip, @gpOur_ID);
         ...
         from nv where datnom=@datnom;
          ...
          -- И запомним сумму:
          insert into #summ(datnom,sp) values(@datnom,@sp);
      ...
      declare CurSell cursor local fast_forward for select Tekid, kol as Qty from NV where datnom=@datnom;
      open CurSell;
      fetch next from CurSell into @TekId,@Qty;
      
      WHILE (@@FETCH_STATUS=0)  BEGIN
        print 'CurSell, @Datnom='+cast(@datnom as varchar(10));
        if @flgRezerv=1 
          update tdVi set 
   ...
   if @KolError=0 begin
     set @TotalSP=(select sum(SP) from #summ);
     if @TotalSP>1500 
        update NC set Done=1
        from #summ inner join NC on #summ.datnom=nc.DatNom;
   end;

...
CREATE PROCEDURE dbo.[RecordSklad3_DEL] @WriteToComman bit,@Ncod int,@Doc_nom varchar(10),
                                 ...
                                 @ind int,@Line int,@DEPTH int,@Volum float,@DefCont int,
                                 @Ncom int OUTPUT,@hTag int OUTPUT
AS
BEGIN
Declare @id int,@Ncom2 int,@oldprice money,@BCode varchar(20),@AddTara int,@srok int,@AddrId int


-- set @DefCont=(Select DCK from DefContract where pin=@Ncod and ContrMain=1 and  ContrTip=1 and Actual=1)  

  
set @srok=(Select srok from DefContract where DCK=@DefCont)
if @Ncom>0
begin
   if @WriteToComman='true' 
  begin
    Update Comman set  summaprice=summaprice+@SPrice,
...
begin
  if @WriteToComman='true' 
  begin
    set @Ncom2=(select IsNull(max(Ncom),0)+1 from Comman);
     insert into Comman (Ncom,Ncod,date,Time,summaprice,summacost,izmen,isprav,remove,
                         ostat,realiz,corr,plata,closdate,srok,op,our_id,doc_nom,doc_date,
                         comp,izmensc,errflag,copyexists,origdate,skMan,grMan,DCK)
    select @Ncom2,Ncod,CONVERT(varchar,getdate(),4),
             ...
             0,0,0,null,@skMan,@grMan,@DefCont
    from Vendors
    where ncod=@Ncod
  end
end
...
begin
  ...
  /*if @hitag=0
    set @hitag=(select max(Hitag)+1 from Nomen)*/
   Insert into Nomen (hitag,name,inactive,nds,price,cost,minp,
  ...
  values (@hitag,@Name,0,@NDS,@price,@cost,@minp,@mpu,@ngrp,@fName,null,null,NULL,0,
          null,null,null,0,@netto,@brutto,6,0,0,@measId,0,@flgWeigth,0,@Volum)
  set @hTag=@hitag
end
 else
  if @NewTov='false' 
  begin
    set @oldprice=(Select price from Nomen where hitag=@hitag)
    if abs(@oldprice-@price)>0.01
    begin
      delete BigPriceList where hitag=@hitag
    end;
    if (Len(@fName)>0) and (@fName is not null) and (@fName!='') 
    begin
      update Nomen set price=@price, cost=@cost, minp=@minp, Nds=@Nds, fname=@fName,
                       VolMinP=@Volum
      where hitag=@hitag
    end
    else
    begin
      update Nomen set price=@price, cost=@cost, minp=@minp, Nds=@Nds,VolMinP=@Volum
      where hitag=@hitag
    end;
  ...
  set @id= (select IsNull(max(Id),0)+1 from tdVi)
  ...
  insert into tdVi (ND,ID,STARTID,NCOM,NCOD,DATEPOST,PRICE,START,STARTTHIS,HITAG,
                     ...
                     GTD,VITR,OUR_ID,WEIGHT,MeasId,OnlyMinP,AddrId,DCK)
  select CONVERT(varchar,getdate(),4),@id,@id,@Ncom2,@Ncod,CONVERT(varchar,getdate(),4),
         ...
         OnlyMinP,IsNull(@AddrId,0),@DefCont
  from Nomen
  where hitag=@hitag
  ...
  -- Добавим в InpDet
   insert into InpDet (nd,ncom,id,hitag,price,cost,kol,sert_id,minp,

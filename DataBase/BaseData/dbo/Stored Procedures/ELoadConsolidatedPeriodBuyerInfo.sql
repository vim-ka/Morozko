CREATE PROCEDURE dbo.ELoadConsolidatedPeriodBuyerInfo
@dt1 datetime,
@dt2 datetime
AS
BEGIN
  ...
  left join AgentList a on a.ag_id=dc.ag_id
  left join agentlist s on s.ag_id=a.sv_ag_id
  left join person pa on pa.p_id=a.p_id
  left join person ps on ps.p_id=s.p_id
  left join deps de on de.DepID=a.depid
  ...
  select ds.dck [pin],
         dc.Our_ID [ourid]
  into #tmp_saldo
  from DailySaldoDck ds
  inner join DefContract dc on dc.dck=ds.dck
  where ds.ND=@dt2
  ...
  select * into #friz from dbo.frizer 

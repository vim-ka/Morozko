CREATE procedure dbo.GenerPricesNSP2b_debug 
  @DCK INT, @SkladList varchar(3000)=NULL,
  @Datnom bigint=0, 
  @Action tinyint=0,
    -- null,0 - сформировать прайс-лист для покуп. DCK
    ...
    -- 5 - сформировать прайс-лист для покуп. DCK,  и свернуть ее по Hitag
    -- 6 - сформировать прайс-лист для поставщика VendDCK и заданного списка Hitag
  @IdList varchar(1000)=null, -- список идов
  @OurID tinyint=0, -- Разрешенный OurID. Если 0, то разрешены все наши фирмы.
  @Ag_ID int=0, -- Код агента для Action=5, и при @ag_id>0 аргумент @DCK игнорируется
  @VendDCK int=0,
  @HitagList varchar(1000)=NULL
...
declare @theNMID int, @theOblID int, @theRNID int, @theDepID int, @theSvID int, 
  ...
  @ctVendor tinyint, @ctGroup tinyint, @ctWares tinyint, @ctAll tinyint,
...
-- Может быть, это покупатель с ответхранения, contrtip=7 ?
set @ContrTip=isnull((select contrtip from defcontract where dck=@DCK),0);
print('@ContrTip='+cast(@ContrTip as varchar));

if @VendDCK is null set @VendDCK=0;
if @Datnom > 0 set @DCK = (select dck from nc where datnom = @DATNOM);
IF isnull(@OurID,0)=0 SET @OurID = ISNULL((SELECT Our_id FROM DefContract WHERE DCK=@DCK),0);
SET @FirmGroup=ISNULL((SELECT FirmGroup FROM FirmsConfig WHERE Our_id=@OurID),0);  

print '@OurID='+cast(@ourid as varchar)+',  @DCK='+cast(@dck as varchar)
...
begin
  --SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
  -- Формирую список активных задач:
  begin try
  
  set @today = dbo.today()
  create table #J (nmid int, prior tinyint);
  insert into #J 
    select nmid, prior from netspec2_main m 
    where m.Activ=1
    and m.StartDate<=@today and m.FinishDate>=@today;

SELECT '#J' AS Rem,* FROM #J;

  create index J_tmp_idx on #J(nmid);
  
  create table #LocalTDVI (ID int, hitag int, Ncod int, Sklad int, Rest decimal(10,3), 
    Our_ID int, price money, cost money, locked bit, minp int, mpu int, weight float, 
    startid int, datepost datetime, DCK int, UnID TINYINT)
  
  declare @saledate datetime;
  set @saledate=dbo.DatnomInDate(@datnom);
  if @Action is null set @Action=0;

  print('@Datnom='+cast(@datnom as varchar)+', соотв.дата='+cast(@saledate as varchar)+', @Action='+cast(@action as varchar));

  if (@Datnom>0) and (@Action=1 or @Action=2) and dbo.DatnomInDate(@datnom)=@today BEGIN
    print('[a1]');
    insert into #LocalTDVI (ID, hitag , Ncod, Sklad, Rest, Our_ID, price, cost, locked, minp, mpu, weight, startid, datepost,DCK, UnID)
      select v.ID, v.hitag , v.Ncod, v.sklad, v.morn-v.sell+v.isprav-v.remov-v.rezerv-v.bad, v.Our_ID, v.price, v.cost, v.Locked, 
        v.minp, v.mpu, v.weight, v.startid, v.datepost,v.DCK, v.UnID
      from tdvi v inner join nv on v.id=nv.tekid 
      where nv.datnom=@Datnom;
  end;

  else if (@Datnom>0) and (@Action=1 or @Action=2) and dbo.DatnomInDate(@datnom)<@today
  begin
    insert into #LocalTDVI (ID, hitag , Ncod, Sklad, Rest, Our_ID, price, cost, locked, minp, mpu, weight, 
      startid, datepost,DCK, UnID)   
    select v.ID, v.hitag , v.Ncod, v.sklad, v.start, v.Our_ID, v.price, v.cost, v.Locked, v.minp, v.mpu, v.weight, 
      v.startid, v.datepost,v.DCK, v.UnID
  ...
  END;
    ...
    select v.ID, v.hitag , v.Ncod, v.sklad, v.morn-v.sell+v.isprav-v.remov-v.rezerv-v.bad, v.Our_ID,
        v.price,
        v.cost,
        v.Locked, v.minp, v.mpu, v.weight, v.startid, v.datepost,v.DCK, v.UnID
    ...
    insert into #LocalTDVI (ID, hitag , Ncod, Sklad, Rest, Our_ID, price, cost, locked, minp, mpu, weight, startid, datepost,DCK, UnID)
    select v.ID, v.hitag , v.Ncod, v.sklad, v.morn-v.sell+v.isprav-v.remov-v.rezerv-v.bad, v.Our_ID, v.price, v.cost, v.Locked, v.minp, v.mpu, v.weight, v.startid, v.datepost,v.DCK, v.UnID
    from  
      tdvi v 
    ...
    insert into #LocalTDVI (ID, hitag , Ncod, Sklad, Rest, Our_ID, price, cost, locked, minp, mpu, weight, startid, datepost,DCK, UnID)
                   select distinct  v.ID, v.hitag , v.Ncod, v.sklad, v.morn-v.sell+v.isprav-v.remov-v.rezerv-v.bad-isnull(z.zakaz,0), v.Our_ID, v.price, v.cost, v.Locked, v.minp, v.mpu, v.weight, v.startid, v.datepost,v.DCK, v.UnID
    ...
    print '@FirmGroup='+cast(@FirmGroup as VARCHAR)

    insert into #LocalTDVI (ID, hitag , Ncod, Sklad, Rest, Our_ID, price, cost, locked, minp, mpu, weight, startid, datepost,DCK, v.UnID)
    select ID, hitag , Ncod, Sklad, morn-sell+isprav-remov-rezerv-iif(bad<0,0,bad), tdVi.Our_ID, price, cost, locked, minp, mpu, weight, startid, datepost,DCK, UnID
    from 
      tdvi 
      JOIN FirmsConfig fc ON fc.Our_id=tdvi.OUR_ID
    where fc.FirmGroup=@FirmGroup      -- так было: and (@OnlyMorozko=0 or tdvi.OUR_ID=7)
...
SELECT '#LocalTDVI' AS tabName, * FROM #LocalTDVI;
...
SELECT '#J' AS [#J], * FROM #J;
SELECT '#LocalDefContract' AS [#LocalDefContract],* FROM #LocalDefContract;
  ...
  union
   ...
   union
    ...
    where 
      dc.Actual=1
      and W.CodeTip=0 -- все вообще покупатели

SELECT '#DetWho' AS [#DetWho], * FROM #DetWho
...
SELECT '#DetWhat' AS [#DetWhat], * FROM #DetWhat;
  ...
  create table #t (id int, sklad int, hitag int, Ncod int, ngrp int, [Parent] int, MainParent int, 
    ...
    Nacen decimal(6,2), nmid int, Disab bit default 0, flgFract bit default 0, 
    Locked bit default 0, MinP int, Mpu int, Rest decimal(10,3), flgMinPrice bit default 1, DCK int default 0, 
    Kol decimal(10,3) default 0, Kol_b decimal(10,3) default 0, Country varchar(50),DateR datetime,SrokH datetime,
    ...
    UnID TINYINT default 0, OrigUnid TINYINT default 0, K DECIMAL(18,10) DEFAULT 1)


  /****************************************************************/
  if @Action=4 -- задан список ID товаров:
    insert into #t (id,sklad,hitag,ncod,ngrp, [Parent], MainParent, WEIGHT,Origprice, flgFract, Cost, Locked,Minp,Mpu, Rest,DCK,nmid,UnID)
    select 
      ...
      v.price, nm.flgFract,V.Cost, v.Locked, v.minp, v.mpu,
      v.Rest, @DCK,
      iif(s.discard=1 or s.Discount=1, 0,null), V.UnID
    from 
 ...
 /****************************************************************/
  ELSE if @Action=6 and @VendDCK>0 -- задан список HITAG товаров:
  begin
 ...
 begin
  ...
  -- SELECT '#t, Action=6' AS [#t],* FROM #t
 END;

  /****************************************************************/
  else if @Action=3
    insert into #t (id,sklad,hitag,ncod,ngrp, [Parent], MainParent, WEIGHT,Origprice, flgFract, Cost, Locked,Minp,Mpu, Rest,DCK,unid)
    select 
...
/****************************************************************/    
  ...
  begin
    insert into #t (id,sklad,hitag,ncod,ngrp, [Parent], MainParent, 
      WEIGHT,
      Origprice, flgFract, Cost, Locked,Minp,Mpu, Rest,DCK,
      Kol,Kol_B,Country,DateR,SrokH,Sert_ID,Gtd,NvId,
      nmid, NvPrice, unid, OrigUnid, K)
    select 
      nv.tekid, nv.sklad, nv.hitag, v.ncod, nm.ngrp, gr.Parent, gr.MainParent, 
      case when v.weight=0 then nm.netto else v.weight end as weight, 
      v.price, nm.flgFract,V.Cost, v.Locked, v.minp, v.mpu,
      v.MORN-v.SELL+v.ISPRAV-v.REMOV-v.REZERV-v.bad as Rest, @DCK,
      nv.kol, nv.kol_b, V.Country,V.DateR,V.SrokH,v.Sert_ID,v.Gtd,nv.NvId,
      iif(s.discard=1 or s.Discount=1, 0,null), nv.Price, nv.UnID, nv.OrigUnid, nv.K
    from 
      nv
...
/****************************************************************/    
  ...
  BEGIN
    PRINT('Эта ветка, Виктор!')
    insert into #t (id,sklad,hitag,ncod,ngrp, [Parent], MainParent, 
      WEIGHT,
      Origprice, flgFract, Cost, Locked,Minp,Mpu, Rest,DCK,Kol,Kol_B,Country,
      DateR,SrokH,Sert_ID,Gtd,NvId, nmid, NvPrice, unid, OrigUnid, K)
    select 
      nv.tekid, nv.sklad, nv.hitag, v.ncod, nm.ngrp, gr.Parent, gr.MainParent, 
      case when v.weight=0 then nm.netto else v.weight end as weight, 
      v.price, nm.flgFract,V.Cost, v.Locked, v.minp, v.mpu,
      v.MORN-v.SELL+v.ISPRAV-v.REMOV-v.REZERV-v.bad as Rest, @DCK,  nv.kol, nv.kol_b, V.Country, 
      v.DateR,v.SrokH,v.Sert_ID,v.Gtd,nv.NvId,
      iif(s.discard=1 or s.Discount=1, 0,null), nv.Price, nv.UnID, nv.OrigUnid, nv.K
    from 
      nv
...
/****************************************************************/    
  ...
  /****************************************************************/      
  else if @Action=0 and @VendDCK=0 begin
    print('Внимание! Отлаживаемая ветка @Action=0, @VendDCK=0');
    ...
    print('Внимание! Запрос выполнен так или иначе. возвращено записей: '+CAST(@Cnt AS VARCHAR));
SELECT '#t-rez' AS [#t-rez],* FROM #t;
   if isnull(@HitagList,'')<>'' delete from #t where Hitag not in (select K from dbo.Str2intarray(@HitagList));
   if isnull(@IdList,'')<>'' delete from #t where Id not in (select K from dbo.Str2intarray(@IdList));
...
/****************************************************************/      
    ...
    select 
      v.id, v.sklad, v.hitag, v.ncod, nm.ngrp, gr.Parent, gr.MainParent, case when v.weight=0 then nm.netto else v.weight end as weight,
      v.price, nm.flgFract,V.Cost, v.Locked, v.minp, v.mpu,
      v.Rest,  C.DCK, iif(s.discard=1 or s.Discount=1, 0,null), V.UnID
    from 
      #LocalTDVI V 
	  inner join #Sk E on E.Sklad=v.SKLAD
      inner join Nomen nm on nm.hitag=v.HITAG
...
-- SELECT '#j' AS J, * FROM #j;
  ...
  where 
    ...
    and #t.flgFract=0 and w.isWeightPrice=0 
    and #t.nmid is null
  ...
  from   
    #t inner join #DetFinal w on #t.hitag=w.hitag and #t.dck=w.dck
  where 
    w.RezTip=@mtFixedPrice --and #t.flgFract=1
  ...
  where 
    w.RezTip=@mtFixedPrice 
    and w.isWeightPrice=0 and #t.flgFract=1
    and #t.nmid is null   
  ...
  where 
    ...
    and w.isWeightPrice=0 and #t.flgFract=0  
    and #t.nmid is null
  ...
  where 
    w.RezTip=@mtAdCostRub
    and w.isWeightPrice=1 and #t.weight<>0 --and #t.flgFract=1
    and #t.nmid is null
  ...
  where 
    w.RezTip=@mtAdPriceRub
    and w.isWeightPrice=1 --and #t.flgFract=1 
    and #t.nmid is null
...
-- SELECT '#detfinal' AS tabName, * FROM #detfinal
-- SELECT '#t' AS tabName, * FROM #t;
  ...
  PRINT('ПОДГОТОВКА РЕЗУЛЬТАТА')
    UPDATE #t SET NewPrice=OrigPrice WHERE NewPrice IS NULL;
-- SELECT * FROM #t;
...
--    select #t.*, nm.[name], nm.FName, case when s.OnlyMinP = 1 then cast(1 as bit) else nm.OnlyMinP end OnlyMinP, nm.Nds,s.UpWeight,nm.netto, #bpl.LastPrice
	select #t.*, nm.[name], nm.FName, s.OnlyMinP, nm.Nds,s.UpWeight,nm.netto, 
    #bpl.LastPrice, u.UnitName
    from #t 
      inner join Nomen NM on nm.hitag=#t.Hitag
      inner join SkladList S on S.SkladNo=#t.Sklad
      inner join Units u on u.unid=#t.unid
      left join #bpl on #bpl.hitag=#t.Hitag
    order by nm.[name], #t.sklad, s.onlyminp, #t.NewPrice desc;
    
  --выгрузка накладной по datnom
  else if isnull(@Action,0)=1 BEGIN
  PRINT('ЧТЕГНИЕ СОДЕРЖИМОГО НАКЛАДНОЙ');
    --  select #t.*, nm.[name], nm.FName, case when s.OnlyMinP = 1 then cast(1 as bit) else nm.OnlyMinP end OnlyMinP, nm.Nds,s.UpWeight,nm.netto,#bpl.LastPrice
    select #t.*, nm.[name], nm.FName, s.OnlyMinP, nm.Nds,s.UpWeight,nm.netto,#bpl.LastPrice, u.unitName
    from #t 
      inner join Nomen NM on nm.hitag=#t.Hitag
      inner join SkladList S on S.SkladNo=#t.Sklad
      left join #bpl on #bpl.hitag=#t.Hitag
...
--    select #t.*, nm.[name], nm.FName, case when s.OnlyMinP = 1 then cast(1 as bit) else nm.OnlyMinP end OnlyMinP, nm.Nds, i.Cost as SourceCost, 
      ...
      nm.Netto, def.brName as fam, V.DatePost, s.UpWeight, #bpl.LastPrice, E.unid
    FROM 
      #t
      inner join Nomen NM on nm.hitag=#t.Hitag

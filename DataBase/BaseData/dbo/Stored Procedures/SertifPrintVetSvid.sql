CREATE PROCEDURE dbo.SertifPrintVetSvid @DatNomStr varchar(300), @uin INT, @total BIT, @dt DATETIME, @Our_ID_Owner INT=0, @Our_IDSaller int=0
AS 
BEGIN 
...
SELECT
  ...
  for xml path(''))
  ,1,2,'') ) AS gpName, 
  IIF(@total=0, iif(NC.B_ID =  57990, 'Воронежская область, г. Воронеж, ул.45 Стрелковой дивизии, 234', Def.gpAddr), '') AS gpAddr,
  STUFF((select '; ' + dbo.SertifCityFromAddr(iif(NC.B_ID =  57990, 'Воронежская область, г. Воронеж, ул.45 Стрелковой дивизии, 234', d.gpAddr))
    FROM Def d WHERE d.pin IN
    (SELECT def.pin
    FROM NC JOIN #NVtemp ON NC.DatNom = #NVtemp.DatNom
          JOIN FirmsConfig fc ON NC.OurID=fc.Our_id
  ...
  for xml path(''))
  ,1,2,'')  AS gpCity,  
   STUFF((select '; ' + dbo.SertifStreetFromAddr(iif(NC.B_ID =  57990, 'Воронежская область, г. Воронеж, ул.45 Стрелковой дивизии, 234', d.gpAddr))
    FROM Def d WHERE d.pin IN
    (SELECT def.pin
    FROM NC JOIN #NVtemp ON NC.DatNom = #NVtemp.DatNom
          JOIN FirmsConfig fc ON NC.OurID=fc.Our_id
  ...
  IIF(nc.Stip=4 AND DefContract.DCK<>45004, ISNULL(d.brName, ''), fc.OurName) AS OurName,
  ...
  isnull(SUM(#NVtemp.Kol*iif(Nomen.flgWeight=0 or #nvTemp.Tekid=-1, Nomen.Netto,tdvi.Weight)),0) as W
  ...
  FROM NC JOIN #NVtemp ON NC.DatNom = #NVtemp.DatNom
          ...
          LEFT JOIN DefContract ON NC.gpOur_ID = DefContract.DCK
          LEFT JOIN Def d ON DefContract.pin = d.pin   
  WHERE NC.SP>=0 
...
GROUP BY 
  ...
  Def.gpName,
  IIF(@total=0, iif(NC.B_ID =  57990, 'Воронежская область, г. Воронеж, ул.45 Стрелковой дивизии, 234', Def.gpAddr), ''),
  Def.Obl_ID,
  usrpwd.fio, 
  Trades.tName,
  fc.Our_id, 

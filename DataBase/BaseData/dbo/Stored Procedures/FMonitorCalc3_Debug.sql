CREATE procedure dbo.FMonitorCalc3_Debug @day0 datetime, @day1 datetime
as
begin
...
select 'Поиск 43214' as Comment,* from #w where pin=43214;


  insert into #w(pin, dck, ag_id) 
    select pin,0 as dck,ag_id from #t where sv_ag_id=278 or ag_id=57; --  or ag_id=234;

  -- Дописываю покупателей, сфотографированных сверх плана:
SELECT 'table #w - 1', * from #w where pin=43214;
  insert into #w(pin,dck, ag_id)
  select distinct #t.pin, dc.dck, dc.ag_id
  from #t left join defcontract dc on dc.pin=#t.pin 
  where #t.pin not in (select pin from #w);
SELECT 'table #w - 2', * from #w where pin=43214;

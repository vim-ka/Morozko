...
CREATE procedure dbo.MarketRequestCalcByAgents @MRID int, @nd0 datetime, @nd1 datetime, @flgDebug bit=0, @skipproclist bit=1 
WITH RECOMPILE
as
...
--print('КОНТРОЛЬНАЯ ТОЧКА 7 ПРОЙДЕНА')
  ...
  insert into MarketRequestRes(mrid, ag_id, b_id, cnt, OBl_ID)
  select 
  ...
  from 
    ...
    inner join MarketRequestDeps D on D.depid=#nkk.depid    
    inner join MarketRequestTovs T on T.hitag=#nkk.hitag
    inner join Def on Def.pin=#nkk.b_id
  where 
    T.bonus=0
    and T.mrid=@mrid 
    and d.mrid=@MRID
  group by     
    #nkk.depid, #nkk.ag_id, #nkk.b_id, def.Obl_ID
  having sum(case 
      when #nkk.tip=0 then #nkk.kol
      when #nkk.tip=1 then #nkk.kol/#nkk.Box
      when #nkk.tip=2 then #nkk.sp

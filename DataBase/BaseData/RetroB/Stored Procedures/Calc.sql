CREATE procedure RetroB.Calc
  @day0 datetime, @day1 datetime, 
  @VedID int,  @RBID int, @Comp varchar(30)='it4', 
  @ClearFlag bit=1  -- сейчас этот параметр не используется
  with recompile
as
declare @WeightFlag smallint,  -- 0-шт, 1-вес, 2-шт+вес
  @BonusPerc decimal(12,2), @rawid int,
    @PayBySell bit,
...
begin
  set @WeightFlag=(select WeightFlag from retrob.rb_Main where rbid=@rbid);
...
if EXISTS(select * from retrob.rb_Main where Active=1 and rbid=@rbid) begin
  ...
  select @period0=m.StartDay, @Period1=m.FinishDay, @PayBySell=m.PayBySell, 
    @Active=m.Active, @flgWoNds=m.flgWoNds, @FirmGroup=m.FirmGroup
  From retrob.rb_main m where m.rbid=@RBID;

  -- Список фирм, входящих в указанную группу фирм, или всех сразу:
  create table #FG(OurID int);
  insert into #FG select distinct our_id from FirmsConfig where FirmGroup=@Firmgroup or @Firmgroup=0;
  create index FG_temp_idx on #FG(OurID);
    ...
    IF @PAYBYSELL=1 BEGIN
        ...
        if exists(select * from retrob.rb_Buyers where rbid=@rbid and Pin>0)
          ...
          From 
            ...
            inner join #FG on #FG.OurID=r.Our_ID
          where 
            r.vedid=@VedID
            and b.RbId=@RBID
            and (r.price>=@koeff*r.Cost or r.MainParent=3)
            and r.datnom between @FirstNnak and @LastNnak
            and (@WeightFlag=2 or (@WeightFlag=0 and r.Weight=0) or (@WeightFlag=1 and r.Weight<>0))
...
-- select 'Это RB_RAW' as remark, * from retrob.rb_raw r where r.vedid=@vedid and r.datnom between 1702010001 and 1702289999;
...
--            and r.PayKoeff<>0
        ...
        create index t_idx_rawid on #t(rawid);
-- SELECT 'ОТЛАДКА' AS REMARK,* FROM #T

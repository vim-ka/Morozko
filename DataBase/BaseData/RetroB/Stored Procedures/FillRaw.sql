CREATE procedure RetroB.FillRaw @vedID int
as  -- В таблицу retrob.Rb_RAW попадут все данные для указанной ведомости: все расходные накладные и все накладные, по которым была оплата
...
begin
  ...
  create table #R(  
    ...
    [Ag_ID] int NULL,
    [Sv_ID] int NULL,
    [DepID] smallint NULL, Our_ID smallint, Weight decimal(10,3) );

  insert into #R(datnom, tekid, b_id,master,Hitag, Kol,Cost,Nds,Price,
    PayKoeff, Ncod, Ngrp, MainParent, dck, ag_id, sv_id, depid, Our_ID,Weight) 
  select
    ...
    nc.dck, dc.ag_id, A.sv_ag_id,  A.DepID, NC.OurID, iif(nm.flgWeight=0,0,v.Weight) as Weight
  from
    #t
  ...
  group by
    ...
    nc.dck, dc.ag_id, A.sv_ag_id,  A.DepID, nc.OurID, nm.flgWeight, v.Weight
  having sum(nv.kol)<>0
  order by nv.DatNom;
  ...
  SELECT @vedID,datnom, tekid, b_id,master,Hitag, Kol,Cost,Nds,Price,
    isnull(PayKoeff,0) as PayKoeff, Ncod, Ngrp, MainParent, dck, ag_id, sv_id, depid, Our_ID,Weight
  from #R;

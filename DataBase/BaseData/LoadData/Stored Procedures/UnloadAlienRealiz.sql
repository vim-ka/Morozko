CREATE PROCEDURE LoadData.UnloadAlienRealiz @Our_ID int, @DateStart datetime, @DateEnd datetime, @AlienSale bit, @Sertif bit
AS
BEGIN
  ...
  select v.hitag,n.ourid, sum(v.kol) as kol, sum(v.kol*v.cost) as sp, sum(v.kol*v.price*(1.0+n.extra/100)) as sl, e.name, d.Obl_ID, d.pin 
  ...
  where n.datnom between @datnomStart and @datnomEnd and isnull(dc.our_id,7)=@Our_ID and n.ourid<>@Our_ID
        ...
        and l.Discard=0 and (isnull(g.Vet,0)=1 or @Sertif = 0)
  group by n.ourid, v.hitag, e.name, d.Obl_ID, d.pin
  having sum(v.kol)<>0
  order by n.ourid, v.hitag
  
  else              --продажи чужого товара через выбранную организацию
  
  select n.nd,v.hitag, n.ourid, sum(v.kol) as kol, sum(v.kol*v.cost) as sp, sum(v.kol*v.price*(1.0+n.extra/100)) as sl, e.name, d.Obl_ID, d.pin
  from  nv v join nc n on v.datnom=n.datnom
             join visual s on v.tekid=s.id
             left join nomen e on v.hitag=e.hitag
             left join gr g on e.ngrp=g.ngrp
  ...
  where n.datnom between @datnomStart and @datnomEnd and isnull(dc.our_id,7)<>@Our_ID and n.ourid=@Our_ID
        and g.MainParent not in (0,84,86,90) and /*(isnull(f.nVet,'')<>'' or @Sertif = 0) and*/ d.worker=0 and n.Stip<>4
        and l.Discard=0 and (isnull(g.Vet,0)=1 or @Sertif = 0)
  group by  n.nd,n.ourid,v.hitag, e.name, d.Obl_ID, d.pin
  having sum(v.kol)<>0

CREATE PROCEDURE [LoadData].CheckRealiz2_old @DateStart datetime, @DateEnd datetime, @our_id int, @datnom int=0 --, @FastBack bit=0
AS
BEGIN
  set transaction isolation level read uncommitted
  declare @datnomStart int, @datnomEnd int, @datnomEndDob int
  
  set @datnomStart = dbo.InDatNom(0,@DateStart)
  set @datnomEnd = dbo.InDatNom(9999,@DateEnd)
  set @datnomEndDob = dbo.InDatNom(9999,dbo.today()-1);
  ...
  select  t.vk,
          ...
          t.nds
  from
  (select iif(c.refdatnom>0, c.refdatnom, c.datnom) as vk,
          c.tm,
          c.nd,
          ...
          v.price*(1+c.extra/100)*(v.kol+
                  isnull((select sum(r.kol) from nv r join nc c1 on r.datnom=c1.datnom and v.tekid=r.tekid                                         
                                            where c1.refdatnom=c.datnom and c1.datnom<@datnomEndDob
                                            and isnull(c1.remark,'')='' and v.datnom=c1.refdatnom),0)
          ) as sm,
          (iif(isnull(s.weight,0)=0, v.kol, v.kol*s.weight)+
                  isnull((select sum(iif(isnull(i.weight,0)=0, r.kol,r.kol*i.weight))
                                             from nv r join nc c1 on r.datnom=c1.datnom 
 ...
 where ((c.datnom>=@datnomStart and c.Datnom<=@datnomEnd and c.refdatnom=0) or (c.refdatnom>=@datnomStart and c.refdatnom<=@datnomEndDob)) 
...
*/
  ...
  (select iif(c.refdatnom>0, c.refdatnom, c.datnom) as vk,
   ...
   from nc c join nv v on c.datnom=v.datnom
             ...
             left join def f on d.master=f.pin
             join nomen n on v.hitag=n.hitag
 where ((c.datnom=@datnom and c.refdatnom=0) or (c.refdatnom=@datnom)) 
       and c.ourid=@our_id and c.Sp>0 and c.Actn<>1 
       and c.Frizer<>1 and c.Tara<>1 and c.STip not in (2,3,4)
  ) t
  where t.kol<>0

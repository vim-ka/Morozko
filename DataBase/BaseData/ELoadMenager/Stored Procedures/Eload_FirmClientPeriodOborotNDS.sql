CREATE PROCEDURE ELoadMenager.Eload_FirmClientPeriodOborotNDS
@dt1 datetime,
@dt2 datetime,
@our_ids varchar(200)
AS
...
begin
  ...
  begin
    ...
    from  def d left join def dnet on d.master=dnet.pin
     ...
     group by iif(dnet.pin>0,dnet.pin, d.pin),iif(dnet.pin>0,dnet.brName, d.brName),n.nds, iif((dc.BnFlag=1 or dc.NeedCK=1),0,1),dc.Our_ID
   end 
...
where c.stip <> 4
      ...
      --and gr.AgInvis=0
group by c.datnom,c.b_id,n.nds,c.OurID,iif((dc.BnFlag=1 or dc.NeedCK=1),0,1)       
...
select n1.period [Период],
       ...
       n1.nal [Наличный],
       n1.sPrice [Продажа, НДС10%],
       n1.sCost [Стоимость закупки, НДС10%],
       n1.sVal [Валовая прибыль, НДС10%],
       n1.sNac [Процент наценки, НДС10%],
       n2.sPrice [Продажа, НДС18%],
       n2.sCost [Стоимость закупки, НДС18%],
       n2.sVal [Валовая прибыль, НДС18%],
       n2.sNac [Процент наценки, НДС18%]
from
(select period ,
   	  #res.our_id,
       ff.OurName,

CREATE PROCEDURE [ELoadMenager].VendAllMove @Ncod int, @StartND datetime, @EndND datetime
AS
BEGIN
...
select year(c.date) as Y,month(c.date) as M, i.hitag ,sum(i.kol*iif(n.flgWeight=1,i.weight,n.netto)) as KGInp
into #comm
...
select year(c.nd) as Y,month(c.nd) as M,v.hitag, sum(v.kol*iif(n.flgWeight=1,i.weight,n.netto)) as Kg7
into #realiz7
...
select year(c.nd) as Y,month(c.nd) as M,v.hitag,sum(v.kol*iif(n.flgWeight=1,i.weight,n.netto)) as KgAll
into #realizAll
from nc c join nv v on v.datnom=c.datnom
          join visual i on v.tekid=i.id
          join nomen n on v.hitag=n.hitag
...
select year(c.nd) as Y,month(c.nd) as M,v.hitag,sum(v.kol*iif(n.flgWeight=1,i.weight,n.netto)) as KgBonus
into #BonusAll
from nc c join nv v on v.datnom=c.datnom
          join visual i on v.tekid=i.id
          join nomen n on v.hitag=n.hitag
...
select year(z.nd) as Y,month(z.nd) as M,z.hitag,sum((z.kol-z.newkol)*iif(n.flgWeight=1,i.weight,n.netto)) as KgSnyat
into #Snyat
...
select year(z.nd) as Y,month(z.nd) as M,z.hitag,sum((z.kol-z.newkol)*iif(n.flgWeight=1,i.weight,n.netto)) as KgIspr
into #Spis
from izmen z join visual i on z.id=i.id
             join nomen n on z.hitag=n.hitag
             join defcontract d on z.dck=d.dck

CREATE PROCEDURE ELoadMenager.[Eload_TransDeliv_new] 
@day0 datetime, 
@day1 datetime,
@ngrp int = 0,
@Tip int = 0 --1 общий
...
begin
  ...
  where c.datnom>=@datnom1 and c.datnom<=@datnom2 and c.frizer=0 and c.tara=0 and /*c.stip<>4 and*/ c.b_id<>56607
        and c.weight<>0
  ...
  select sum(c.weight) [weight],
         sum(c.sp) [sp],
         sum(c.sc) [sc], 
  		 sum(iif(v.crid=7,isnull(v.expense,0)+iif(m.Dist>0,m.Dist,m.CalcDist)*isnull(v.tariff1km,0),0))/count(c.datnom) [rashod],
  		 count(c.datnom),
         c.mhid         
  from #nc c join marsh m on c.mhid=m.mhid
  ...
  update #s set rashod=rashod+(select sum(m.oplatasum+m.percworkpay+m.bonus) from nearlogistic.nllistpaydet m where m.mhid=#s.mhid)
  ...
  select @day0 [day0],
         ...
         e.depid,
         e.crid,
         iif(@Tip=0,iif(e.crid=7,'Морозко','Сторонние'),'Общий') [crName],
         d.dname,
         sum(e.weight) [weight],

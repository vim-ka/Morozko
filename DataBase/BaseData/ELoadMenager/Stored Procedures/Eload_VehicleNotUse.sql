CREATE PROCEDURE [ELoadMenager].Eload_VehicleNotUse
@nd datetime
as 
begin
...
from dbo.Vehicle v 
where v.Closed=0
			and not v.v_id in (select m.v_id from dbo.marsh m where nd=@nd and isnull(m.v_id,0)>0)
      and v.trailer=0
...
where d.trid in (6,7) and isnull(d.v_id,0)>0 and d.Closed=0
order by 3 desc, 1

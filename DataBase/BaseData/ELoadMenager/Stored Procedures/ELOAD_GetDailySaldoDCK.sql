CREATE PROCEDURE ELoadMenager.ELOAD_GetDailySaldoDCK
@nd DATETIME, 
@our_id INT,
@isGroup BIT
AS 
...
INNER JOIN dbo.def d ON dc.pin = d.pin
INNER JOIN #ourFIRMS fc ON dc.Our_id = fc.Our_id
LEFT JOIN dbo.AgentList al ON dc.ag_id = al.AG_ID
LEFT JOIN dbo.Person p ON al.P_ID = p.P_ID
WHERE dc.Actual=1   
...
SELECT sd.DCK [КодДоговора],
       sd.DCKNAME [НаименованиеДоговора],
       sd.PIN [КодТочки],
       sd.PINNAME [НаименованиеТочки],
       d.gpAddr as [Адрес],
       d.gpPhone as [Контакт],
       sd.OURID [КодФирмы],
       sd.OURNAME [НаименованиеФирмы],
       sd.AG_ID [КодАгента],
       sd.AGNAME [ФИОАгента],
       sd.MUST [Долг],
       sd.DEEP [Просрочка, дн.],
       sd.OVERDUE [Просрочка, руб.],
       sd.SMWEEK [Сумма оплат за предыдущие 7 дней, руб.],
       dc.BnFlag as [Безналичный расчет],
       d.brINN as [ИНН],
       d1.depid [Код отдела],
       d1.DName [Отдел]
from #tmpSaldoDCK sd INNER JOIN dbo.def d ON d.pin=sd.pin
                     INNER JOIN dbo.DefContract dc ON dc.dck=sd.DCK
                     LEFT JOIN dbo.AgentList al ON al.ag_id = iif(dc.ag_id=17 or dc.ag_id=32 or dc.ag_id=33 or dc.ag_id=641, dc.PrevAG_ID, sd.AG_ID)
                     LEFT JOIN dbo.Deps d1 ON al.DepID = d1.DepID
...
INNER JOIN dbo.def d ON d.pin=x.MASTER
INNER JOIN dbo.DefContract dc ON dc.dck=x.DCK
LEFT JOIN dbo.AgentList al ON dc.ag_id = al.AG_ID
LEFT JOIN dbo.Deps d1 ON al.DepID = d1.DepID
ORDER BY 4,2*/

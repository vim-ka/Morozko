CREATE procedure [Salary].[CalcSalary_DEBUG] @day0 datetime, @day1 datetime, 
...
begin
  ...
  insert into #t(datnom, dck, b_id, ag_id, plata,sp,sc, extra)
    ...
    where k.oper=-2 and 
    ((k.bankday is null and k.nd between @day0 and @day1) or (k.bankday is not null and k.bankday between @day0 and @day1))
  ...
  if @RecalcMode='Full' begin
    ...
    exec SaveDailySaldoDCK @day1;  
    
    delete from salary.Salary2buyers where s2id=@s2id;
    
    insert into salary.Salary2buyers(s2id, b_id, dck, ag_id,debt, overdue, overup17, Plata, Sell)
    ...
    delete from salary.Salary2agentlist where s2id=@s2id;
    
    insert into Salary.Salary2agentlist(s2id, ag_id, AgentFam, sv_Ag_ID, DepID)
    select distinct
      @s2id, A.ag_id, p.fio as AgentFam, a.sv_ag_id, Sv.depId 
    from 
      salary.Salary2buyers b
    ...
    delete from salary.Salary2superlist where s2id=@s2id;

    insert into Salary.Salary2superlist(s2id, sv_ID, SuperFam, DepID)
    select distinct
      @s2id, sv.ag_id as sv_iD, p.fio as SuperFam, Sv.depId 
    from 
 	  Salary.Salary2agentlist A
      inner join agentlist sv on sv.ag_id=a.sv_Ag_ID
  ...
  if not exists(select * from salary.Salary2superlist where s2id=@s2id)
    insert into Salary.Salary2superlist(s2id, sv_ID, SuperFam, DepID)
    select distinct
      @s2id, sv.ag_id as sv_iD, p.fio as SuperFam, Sv.depId 
    from 
  ...
  if @RecalcMode<>'none' begin
    delete from Salary.Salary2result where s2id=@s2id; 

    insert into salary.Salary2result
    select 
      @s2id,
      sv.DepID, Deps.dname,
      a.sv_ag_id as SV_id, 
    ...
    from 
      ...
      inner join salary.Salary2superlist sv on sv.sv_id=a.sv_ag_id
...
/*
CREATE procedure Salary.CalcSalary @day0 datetime, @day1 datetime, 
  @RecalcMode varchar(20), -- режим пересчета:
  -- "Full" - полный, "Pay" - только выплат.
  @Cname varchar(20), @yy int, @mm int
as
...
begin
  ...
  -- Какие расходные накладные (без выплат!) нужто еще учесть в расчете?
  insert into #t(datnom, dck, b_id, ag_id, plata,sp,sc, extra,part)
  select nc.datnom, nc.dck, nc.B_ID, dc.ag_id, 0 as Plata,  nc.sp, nc.sc, nc.extra,0
  from 
    nc 
  ...
  if @RecalcMode='Full' begin
    ...
    from 
      Agentlist A 
      left join salary.Salary2buyers B on A.ag_id=b.ag_id and B.s2id=@s2id
      inner join Person P on p.p_id=a.p_id
      inner join agentlist sv on sv.ag_id=a.sv_Ag_ID
  end;
  ...
  if @RecalcMode<>'none' begin
    ...
    select 
      @s2id,
      sv.DepID, Deps.dname,
      a.sv_ag_id as SV_id, sv.AgentFam as SuperFam,
      #t.ag_id, a.AgentFam,    
      #t.b_id, #t.dck, Def.gpName,
      b.plata, --  sum(#t.plata) as Plata,
      sum(#t.part*nv.kol*(NV.Price*(1.0+#t.extra/100) - nv.Cost)) as Dohod,
    ...
    from 
      ...
      inner join Nomen nm on nm.hitag=nv.hitag
      inner join GR on GR.Ngrp=nm.ngrp
      left  join Salary.agentmatrix m on  gr.MainParent=m.ngrp and m.ag_id=#t.ag_id
      inner join Def on Def.pin=#t.b_id
      left  join salary.Salary2buyers b on b.dck=#t.dck and b.b_id=#t.b_id
    where
      m.yy=2013 and m.mm=@MM 
      and a.s2id=@s2id
      and sv.s2id=@s2id
      and b.s2id=@s2id
    group by
      sv.DepID, Deps.dname, a.sv_ag_id, sv.AgentFam, #t.ag_id, a.AgentFam,
      #t.b_id, #t.dck, Def.gpName, b.plata,b.Sell
  end;

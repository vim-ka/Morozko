CREATE procedure Guard.CalcOverDet_Debug @nd datetime, @Deep int=30, @PrevSunday datetime
as
...
begin
  ...
  --  create table #pr (dck int, ag_id int, Overdue decimal(10,2), Deep int, LastSundayOver decimal(10,2) default 0);
  --  insert into #pr(dck,ag_id,overdue,deep)
  --  SELECT 
  --    nc.dck, iif(dc.ag_id in (33,641) and dc.prevag_id is not null, dc.PrevAg_ID, dc.ag_id) as AG_ID,
  --    sum(nc.sp-isnull(#p.plata,0)+ISNULL(#Z.delta,0)) as Overdue,
  --    max(DATEDIFF(day,dateadd(day, nc.srok, nc.nd), @ND)) as Deep
  --  from 
  --    nc
  ...
  --  where 
  --    nc.nd<=@nd
  --    and nc.srok>0 and nc.frizer=0 and nc.Actn=0
  --    and DATEDIFF(day,dateadd(day, nc.srok, nc.nd), @ND)>=@Deep
  --    and dc.ag_id<>17
  --  group by 
  --    nc.dck, iif(dc.ag_id in (33,641) and dc.prevag_id is not null, dc.PrevAg_ID, dc.ag_id)
  --  having sum(nc.sp-isnull(#p.plata,0)+ISNULL(#Z.delta,0))>1.00
  --  order by nc.dck;  
...
select '#R' as remark,* from #r where b_id in (select pin from def where Master=592);
...
select '#R-2' as remark,* from #r where b_id=592 or MasterPin=592;
...
select 'Вставка в #PZ' as remark, max(#r.depid) depid, max(#r.sv_ag_id) sv_ag_id,  #r.MasterPin as b_id, #r.MasterDCK as dck, 
...
select '#PZ' as remark,* from #pz where #pz.b_id=592 order by depid, Deep desc, svfam, gpname;


  truncate table Guard.OverDet;
  
  INSERT INTO Guard.OverDet(DepId,DName,sv_ag_id,svFam,b_id,dck,gpname,TekOver,Deep,
    LastWeekOver,remark,Plata,Mess,IncomePlan,FrizQty,LastDay,LastPay) 
  select DepId,DName,sv_ag_id,svFam,b_id,dck,gpname,TekOver,Deep,
    LastWeekOver,remark,Plata,Mess,IncomePlan,FrizQty,LastDay,LastPay
  from #pz;

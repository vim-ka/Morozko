CREATE procedure Guard.PrepareData @ND datetime
AS
declare @dn smallint
begin   
...
-- SELECT * FROM #PLANB WHERE SV_AG_ID=24;
  ...
  if not exists(select * from #a where sv_ag_id=871)
    insert into #a select ag_id, 871 from agentlist where sv_ag_id=871;

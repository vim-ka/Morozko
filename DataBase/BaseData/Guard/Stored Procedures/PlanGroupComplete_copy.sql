...
CREATE procedure Guard.PlanGroupComplete_copy @Who int, @Mode tinyint, @ND datetime, @Ngrp smallint=92
...
select * from #br where b_id=1020;
...
from
  ...
  left join (select distinct b_id from #BR) E on E.b_id=N.pin
where
...
SELECT * FROM #LQ where b_id=1020

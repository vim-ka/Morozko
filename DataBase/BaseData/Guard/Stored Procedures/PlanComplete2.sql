/* 
Контракты выкинуты из результата, всё свернуто по покупателям, агентам, супервайзерам, отделам.

Надо как следует продумать, что мы хотим получить на выходе.

Исходные данные:
- имеется список выплат покупателей, список фотографий, список аудитов, список продаж, список оплат.
- каждое такое событие засчитывается за посещение. С оговорками.
- привязка покупателей к агентам выполняется не обычным способом (через def-defcontract-agentlist),
  а через таблицу запланированных посещений PlanVisit2.
- тем не менее некоторые посещения выполняются в обход плана. И вот их надо зачитывать обычным способом.
 */


CREATE procedure Guard.PlanComplete2
   @ag_id int, @Mode tinyint, @NeedDay datetime, @Comp varchar(30)=null
...
begin
  ...
  -- ***************************************************************************************************
    ...
    insert into #br(dck,b_id,Plann,ag_id, dt)
    select 
      p.dck, p.pin, 1 as Plann, p.ag_id, dbo.fnMinutes2str(min(p.tm))
    from
      planvisit2 P 
...
-- SELECT 'Данные агента 748' AS REM,* FROM #BR WHERE AG_ID=798;
    ...
    where 
      ...
      and fm.dck>0
-- SELECT 'То же плюс прод/опл/фото' AS REM,* FROM #BR WHERE AG_ID=798;
  ...
  if @Comp is null
    select @Comp as Comp, #br.b_id, max(#br.plann) plann, #br.ag_id,#br.dt,#br.BName, max(#br.factT) FactT, #br.Tara, 
    ...
    group by #br.b_id, #br.ag_id,#br.dt,#br.BName,  #br.Tara, 
      #br.Audit, #br.AdvOrd,  #br.NeudSpr, #br.LastSver, #br.LastSell, 
    ...
    INSERT INTO Guard.PlanExec(
      plann, Comp,  dck,  b_id,  ag_id, dt, Tip, BName,  FactT, SkipT,  Tara,  Audit,
    ...
    select 
      0 as Plann, @Comp, 0 as DCK, #br.b_id, #br.ag_id, max(isnull(dt,'')) dt, #br.tip, #br.BName, 
      max(isnull(#br.factT,'')) FactT, max(isnull(#br.SkipT,'')), sum(#br.Tara), max(isnull(#br.Audit,'')), 
      max(isnull(#br.AdvOrd,'')),  sum(isnull(#br.NeudSpr,0)), max(#br.LastSver), 
      max(isnull(#br.LastSell,'20000101')),  max(isnull(#br.DayProd,'')), sum(#br.Debt),      
      sum(iif(#br.Overdue<0,0,#br.Overdue)) Overdue,  
      sum(iif(#br.Over17<0,0,#br.Over17)) Over17,   max(isnull(#br.Deep,0)),  
      sum(isnull(#wp.WeekSell,0)), sum(isnull(#mp.MonthSell,0)),
      sum(isnull(#wm.WeekPay,0)), sum(isnull(#mm.MonthPay,0)), 
      A.DepID, A.sv_ag_id,  P.Fio as SuperFam,
      sum(isnull(#k.plata,0)) as DayPay,  sum(isnull(#f.qty,0)) as FrizQty, def.gpAddr, 
      max(isnull(#AL.LastSverSVDate, 0)),
      max(isnull(#AL.LastSverSVState,0)),      
      sum(isnull(#PH.Photos,0)),       
      PA.Fio, sum(isnull(#dp.todaysell,0))
    from 
      #br 
      left join #k on #k.dck=#br.dck and #k.nd=@NeedDay
      left join #wp on #wp.dck=#br.dck
    ...
    where 
      (#br.overdue>0 and #br.deep>=17) or @Mode<3
    group by 
      #br.b_id,#br.ag_id, #br.tip, #br.BName, 
      def.gpAddr,  A.DepID, A.sv_ag_id,  P.Fio,
      PA.Fio
    order by A.sv_ag_id, ag_id, #br.bname, b_id;

    select top 1000 * from guard.planexec where Comp=@Comp order by b_id;
  end;

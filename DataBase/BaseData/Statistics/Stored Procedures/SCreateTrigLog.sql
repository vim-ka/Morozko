CREATE procedure [Statistics].SCreateTrigLog (@TableName VARCHAR(128)) 
as
...
begin
  ...
  begin 
    ...
    set @TN='ALTER TABLE '+@TableName+'Log  DROP COLUMN ID';
    EXECUTE (@TN);
    
    --столбцы для журналирования
    set @TN=' ALTER TABLE '+@TableName+'Log ADD [LogID] int  IDENTITY(1, 1) NOT NULL; '+ 
            ' ALTER TABLE '+@TableName+'Log ADD [ID] int  NOT NULL; '+ 
            ' ALTER TABLE '+@TableName+'Log ADD [type] smallint NULL; '+
    ...
    EXECUTE (' EXEC sp_addextendedproperty ''MS_Description'', N''тип изменения 0 - insert, 1 - delete, 2 - update'', N''schema'', N''statistics'', N''table'', N'''+@TableName+'Log'', N''column'', N''type''; ');
    EXECUTE (' EXEC sp_addextendedproperty ''MS_Description'', N''имя пользователя'', N''schema'', N''statistics'', N''table'', N'''+@TableName+'Log'', N''column'', N''user_name'';');
    EXECUTE (' EXEC sp_addextendedproperty ''MS_Description'', N''время изменения'', N''schema'', N''statistics'', N''table'', N'''+@TableName+'Log'', N''column'', N''datetime'';');  
    EXECUTE (' EXEC sp_addextendedproperty ''MS_Description'', N''имя компа'', N''schema'', N''statistics'', N''table'', N'''+@TableName+'Log'', N''column'', N''host_name'';');
  	EXECUTE (' EXEC sp_addextendedproperty ''MS_Description'', N''имя приложения'', N''schema'', N''statistics'', N''table'', N'''+@TableName+'Log'', N''column'', N''app_name'';');

CREATE procedure MobAgents.MobSaveZakaz_DEBUG
  @B_ID int, 
	@DCK int, 
	@CompName varchar(30), 
	@Tip varchar(40), 
    ...
    begin
      ...
      select 
        ...
        case when (SL.UpWeight=1) and (n.flgWeight=1) 
             then case when n.netto=0 then round((v.morn-v.sell+v.isprav-v.remov-v.bad-v.rezerv)*v.[WEIGHT],0)
                                      else round((v.morn-v.sell+v.isprav-v.remov-v.bad-v.rezerv)*v.[WEIGHT]/n.netto,0) end
             else
...
print('@DepID='+cast(@Depid as varchar)+'  @FirmGroup='+cast(@FirmGroup as varchar)+'  @PLID='+cast(@plid as varchar))
      open CurRest;
PRINT('КУРСОР CURREST ОТКРЫТ')
      fetch next from CurRest into @tekid, @Sklad, @Ncod, @PriceVI, @Cost, @Rest, @AlienReq, @EffWeight, @Netto, @NDS, @flgWeight;
PRINT('  @TEKID='+CAST(@TEKID AS VARCHAR)+',  @REST='+CAST(@REST AS VARCHAR)+', @QtyZakaz='+cast(@QtyZakaz as varchar))
      set @SavedZakaz=0;
      ...
      close CurRest;
      deallocate CurRest;  
PRINT('КУРСОР CURREST ЗАКРЫТ, @SAVEDZAKAZ='+CAST(@SAVEDZAKAZ AS VARCHAR))
      select @SavedZakaz
    end
    end
  end --provodnoy

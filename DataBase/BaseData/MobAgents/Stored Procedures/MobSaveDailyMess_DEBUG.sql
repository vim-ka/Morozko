CREATE PROCEDURE [MobAgents].[MobSaveDailyMess_DEBUG]  with recompile
AS
BEGIN
...
UNION
  ...
  from (
    ...
    where -- Переделано 30.05.2017 - Виктор
      ...
      and (
           (a.DepID=3 and dc.ag_id>0 and srokh<@nd+9 or DATEDIFF(day, c.ND, @ND)>=20 and v.hitag in (28715,28242,28241,28240,28583,28582))--эпика 
        or (dc.ag_id in (13,486,706,828,957) and srokh<@nd+7  and g.ngrp in (44,85,91,92,105,107,120,122,124,125)) -- колбаса  
        ...
        or (dc.ag_id in (145,210) and DATEDIFF(day, c.nd, @ND)<=4 and g.ngrp in (44,85,91,92,96,105,107,120,122,124,125) )  -- колбаса и торты  
        or (dc.ag_id=845 and srokh<@nd+5 and dc.ag_id>0 and g.ngrp in (44,85,91,92,105,107,120,122,124,125) and c.b_id in (40340,40343,19491,287770,35482))
      )
  ...
  UNION
    ...
    from  nc c join nv v on c.datnom=v.datnom
             ...
             join agentlist a on dc.ag_id=a.ag_id
    -- ТАК БЫЛО:
    -- where  DATEDIFF(DAY, c.ND, @ND)<=200
    --       and pl.dn=@NumND   
    --       and v.kol-v.kol_b>0
    --       and ((a.DepID=3 and dc.ag_id>0 and (DATEDIFF(day, i.srokh, @ND)<=9 or DATEDIFF(day, c.ND, @ND)>=20) and v.hitag in (28715,28242,28241,28240,28583,28582,29235))--эпика 
    --            or (dc.ag_id in (13,486,706,828,957) and DATEDIFF(day, i.srokh, @ND)<=7 and g.ngrp in (85)) --колбаса  
    --            or (dc.ag_id in (13,486,706,828,957) and DATEDIFF(day, c.nd, @ND)<=4 and g.ngrp in (34))     --торты  
    --            or (a.DepID=37 and dc.ag_id>0 and (((DATEDIFF(day, i.srokh, @ND)<=6) and g.ngrp in (85)) or ((DATEDIFF(day, i.srokh, @ND)<=6) and g.ngrp in (96))))      --пикалова 
    --            or (a.DepID=4 and dc.ag_id>0 and (DATEDIFF(day, i.srokh, @ND)<=5) and g.ngrp in (85))       --БИК колбаса
    --            or (dc.ag_id in (145,210) and DATEDIFF(day, c.nd, @ND)<=4 and g.ngrp in (85,96) )     --торты  
    --            or (dc.ag_id=845 and DATEDIFF(day, i.srokh, @ND)<=5 and dc.ag_id>0 and g.ngrp in (85) and c.b_id in (40340,40343,19491,287770,35482))))
    where -- Переделано 30.05.2017 - Виктор
      @nd >= c.ND-200
      and a.DepID<>37
      and pl.dn=@NumND   
      ...
      and ((a.DepID=3 and dc.ag_id>0 and ((g.ngrp in (34) and DATEDIFF(day, c.nd, @ND)<=3))  
            or ((srokh<@nd+9 or DATEDIFF(day, c.ND, @ND)>=20) and v.hitag in (28715,28242,28241,28240,28583,28582)   ))--эпика 
        --or (dc.ag_id in (13,486,706,828,957) and srokh<@nd+7  and g.ngrp in (44,85,91,92,105,107,120,122,124,125)) -- колбаса  
        or (srokh<=@nd+6  and g.ngrp in (44,85,91,92,105,107,120,122,124,125)) -- колбаса  
        or (dc.ag_id in (13,486,706,828,957) and DATEDIFF(day, c.nd, @ND)<=4 and g.ngrp in (34))                   -- торты  
        or (a.DepID=37 and dc.ag_id>0 and srokh<@nd+6 and g.ngrp in (44,85,91,92,96,105,107,120,122,124,125))      -- Пикалова, колбаса и торты  
        or (a.DepID=4 and dc.ag_id>0 and srokh<@nd+5 and g.ngrp in  (44,85,91,92,105,107,120,122,124,125))         -- БИК колбаса
        --or (dc.ag_id in (145,210) and DATEDIFF(day, c.nd, @ND)<=4 and g.ngrp in (44,85,91,92,96,105,107,120,122,124,125) )  -- колбаса и торты  
  ...
  ) E group by e.tip, e.ag_id, e.pin, e.gpname,e.hitag,e.name, e.sp, e.plata, e.FolderName

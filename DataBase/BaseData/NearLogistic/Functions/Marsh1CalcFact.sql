CREATE FUNCTION NearLogistic.Marsh1CalcFact (
  @mhID int, @mode int=1, @DistR decimal(10,3)=0.0 /*для перевозчика - 1, для заказчика - 2*/
)
RETURNS money
AS
BEGIN

declare @smdrv money, @smspd money, @Ret MONEY, @ttID int
...
set @ttid=(select t.ttid 
           from nearlogistic.nltariffParams p join nearlogistic.nltariffsdet d on d.nltariffparamsid=p.nltariffparamsid
                                             ...
                                             join marsh m on p.nlTariffParamsID=m.nlTariffParamsIDDrv
           where m.mhid=@mhid)
...
select 
 cast(isnull(pd.Pay1Km*@DistForCalc+
  ...
  pd.Bonus*@isBonus+
  isnull(h.expense,0)+
  @DistForCalc*isnull(h.tariff1km,0)
  as varchar) as sm
from  
...
where m.mhID=@mhID   and  pd.nlTariffParamsID=m.nlTariffParamsIDSpd  
)


--set @CrutchPay = [NearLogistic].fnCrutch(@mhid, @mode, @DistR)

if @mode=1 
  set @Ret=isnull(@smdrv,0) + isnull(@smspd,0)-- + isnull(@CrutchPay,0)
else if @mode=2
  set @Ret=isnull(@CrutchPay,0) 

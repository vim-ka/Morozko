CREATE PROCEDURE [NearLogistic].MarshList_del 
@ND datetime
AS
BEGIN
SET NOCOUNT ON;
...
select msh.Nd,msh.marsh,
       ...
       (select count(distinct (case when isnull(d.vmaster,0)>0 then d.vmaster else d.pin end)) 
                ...
                where c.nd=msh.nd and c.marsh=msh.marsh) as Dots,
       Dohod as SumZgruz,
       Marja as Nac,
       ...
       IsNull(R.cntCD,0) as DelivC,
        case
          when (S.physPerson=0) and (C.crId>0) and (C.crId<>7) and (S.NDS=0) then 1    
...
from dbo.Marsh  msh
     ...
     left join (select count(datNom) as cntCD, nc.Marsh from NC where nc.Nd= @nd and nc.DelivCancel=1 group by marsh)R on R.marsh=msh.marsh
where msh.nd= @nd
order by Marsh;

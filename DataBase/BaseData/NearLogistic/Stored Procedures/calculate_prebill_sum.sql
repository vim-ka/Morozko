CREATE procedure NearLogistic.calculate_prebill_sum @mhid int, @debug bit =0, @error int out, @msg varchar(500) out
...
set @error=0; set @msg='';	set @tranname='NearLogistic.calculate_prebill_sum'+cast(@mhid as varchar);
...
where r.mhid=@mhid and r.reqtype=-2 
  --and f.cost=0
  ) a
...
join #distance d on d.row_id between (select min(a.row_id) 
                                        from #distance a 
                                       where a.p1=r.p1) and (select min(a.row_id) from #distance a where a.p2=r.p2)
...
BEGIN
  ...
  delete from nearlogistic.billsSum where mhid=@mhid
...
/*
-----вставка в таблицу billsSUM------------------------------------------------------------------------
    insert into nearlogistic.billsSum (mhid, casher_id, distance, is_old, tax, req_pay, auto)
    SELECT @mhid, t.casher_id, SUM(t.km), 
           t.is_old, SUM(t.tax), SUM(t.req_pay), 1
      FROM 
  	(select casher_id, sum(km) AS km, 
           is_old, sum(tax) AS tax, sum(req_pay) AS req_pay
  	from #result 
    group by reqid, casher_id, p1, p2, is_old) t
    GROUP BY t.casher_id, t.is_old
-------------------------------------------------------------------------------------------------------
*/
    ...
    begin
      ...
      select -1 [reqid], a.casher_id, 0 [mas], 0 [vol], d.p1, d.p2, 999 [ord], a.is_old, 999 [row_id], 
             d.p1 [distance_p1], d.p2 [distance_p1], d.km, d.tax / @count_casher [tax], 
...
/*
--------вставка в таблицу billsSUM---------------------------------------
      insert into nearlogistic.billsSum(mhid, casher_id,  distance, is_old, tax, req_pay, nal, auto)
        SELECT t.mhID, t.pin, 0, 
             0, 0, SUM(t.cost), t.nal, 1
          FROM 
      (SELECT f.mhid, f.pin,
              f.cost, f.nal
        from nearlogistic.marshrequests_free f
        join dbo.marsh m on m.mhid=f.mhid
        where f.mhid=@mhid and f.cost<>0) t
        GROUP BY t.mhid, t.pin, t.nal     
-------------------------------------------------------------------------
*/


--------вставка в таблицу billsSUM---------------------------------------
    insert into nearlogistic.billsSum (mhid, casher_id, distance, realdist, is_old, tax, req_pay, auto)
    SELECT @mhid, b.casher_id, 
           SUM(ISNULL(b.distance,0)), 
           NearLogistic.GetRealDistance(@mhid, SUM(ISNULL(b.distance,0))),
           b.is_old, SUM(b.tax), SUM(b.req_pay), 1
      FROM NearLogistic.bills b
     WHERE b.mhid = @mhid
     GROUP BY b.casher_id, b.is_old, b.nal
-------------------------------------------------------------------------
  ...
  commit tran @tranname;    --WITH (DELAYED_DURABILITY = on); 

  END 
	else rollback tran @tranname
end

IF @error=0 or @fixed_req=1 EXEC NearLogistic.TariffClientFind @mhid, 0, 1 
